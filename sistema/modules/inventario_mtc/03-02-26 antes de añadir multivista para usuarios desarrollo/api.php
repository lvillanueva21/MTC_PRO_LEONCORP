  ...
  switch ($action) {
    ...
    case 'get': {
      ...
      $doUpdate = function($targetId) use ($mysqli, $table, $def, $data, $empresaId, $isSingleton) {
        ...
        // En categorías únicas, si este queda activo, desactiva los demás (por si existían duplicados históricos)
      ...
      if ($id > 0) {
        ...
        // En categorías "únicas por empresa": si ya existe uno, se actualiza ese (no se crea otro)
        if ($isSingleton) {
      ...
      // Si es categoría única y se activa uno, se desactivan los demás (por si había duplicados históricos)
