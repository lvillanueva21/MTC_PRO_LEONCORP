...
(() => {
  ...
  const API = CFG.api;
  const IS_MULTI = !!(+CFG.is_multi);

  const state = {
    empresa_id: parseInt(CFG.empresa_id, 10) || 0,
    empresa_nom: (CFG.empresa_nom || '').toString(),
    cat: 'computadoras',
    meta: null,
  ...
  const SINGLETON_CATS = new Set(['red','transmision','dvrs','switches']);
  let isSaving = false;
  ...
  const selEmpresa = $('#inv-empresa');
  const lblEmpresa = $('#inv-empresa-nom');
  const lnkPrint   = $('#lnk-print');
  ...
  function withEmpresa(paramsObj){
    const p = { ...paramsObj };
    if (IS_MULTI && state.empresa_id) p.empresa_id = String(state.empresa_id);
    return p;
  }

  function updateEmpresaUI(){
    if (lblEmpresa) lblEmpresa.textContent = state.empresa_nom || '';
    if (lnkPrint) {
      const base = `${CFG.base}/modules/inventario_mtc/print.php`;
      if (IS_MULTI && state.empresa_id) lnkPrint.href = `${base}?empresa_id=${encodeURIComponent(String(state.empresa_id))}`;
      else lnkPrint.href = base;
    }
  }

  // ---------- Empresas (solo multi) ----------
  async function loadEmpresas(){
    if (!IS_MULTI || !selEmpresa) return;
    try{
      const d = await j(`${API}?action=empresas&empresa_id=${encodeURIComponent(String(state.empresa_id))}`);
      const rows = d.data || [];
      selEmpresa.innerHTML = rows.map(r => {
        const id = parseInt(r.id,10) || 0;
        const nom = (r.nombre || '').toString();
        const sel = (id === state.empresa_id) ? ' selected' : '';
        return `<option value="${id}"${sel}>${esc(nom)}</option>`;
      }).join('');

      // Ajusta nombre visible si en DB cambió
      const opt = selEmpresa.querySelector('option:checked');
      state.empresa_nom = opt ? opt.textContent : state.empresa_nom;
      updateEmpresaUI();
    }catch(err){
  ...
  if (IS_MULTI && selEmpresa) {
    selEmpresa.addEventListener('change', async () => {
      const id = parseInt(selEmpresa.value, 10) || 0;
      const opt = selEmpresa.querySelector('option:checked');
      const nom = opt ? opt.textContent : '';
      if (!id) return;

      state.empresa_id = id;
      state.empresa_nom = nom;
  ...
  async function loadMeta(){
    const qs = new URLSearchParams(withEmpresa({action:'meta'}));
    const d = await j(`${API}?${qs.toString()}`);
    state.meta = d.data || {};
  }

  async function loadStats(){
    const qs = new URLSearchParams(withEmpresa({action:'stats'}));
    const d = await j(`${API}?${qs.toString()}`);
    const s = d.data || {};
    const label = {
  ...
  btnNew.addEventListener('click', () => {
    if (SINGLETON_CATS.has(state.cat)) {
      openSingletonOrNew(state.cat);
      return;
    }
  ...
  async function loadList(){
    try{
      ...
      const qs = new URLSearchParams(withEmpresa({
  ...
  tBody.addEventListener('click', async (e)=>{
    ...
    if (act==='edit') {
      ...
      fd.append('tabla',state.cat);
      fd.append('id', id);
      fd.append('activo', nuevo);
      if (IS_MULTI && state.empresa_id) fd.append('empresa_id', String(state.empresa_id));
  ...
  // ---------- singleton: si ya existe, abre el existente ----------
  async function openSingletonOrNew(cat){
    try{
      const qs = new URLSearchParams(withEmpresa({
        action:'list', tabla: cat,
        page: 1, per: 1,
        q: '', estado: ''
      }));
      const d = await j(`${API}?${qs.toString()}`);
      const row = (d.data && d.data[0]) ? d.data[0] : null;
      if (row && row.id) {
        openForm(cat, parseInt(row.id,10) || 0, null);
        return;
      }
    }catch(err){
      // si falla, caemos a nuevo
    }
    openForm(cat, 0, null);
  }
  ...
  async function openForm(cat, id=0, preset=null){
    ...
    if (id>0) {
      try{
        const qs = new URLSearchParams(withEmpresa({action:'get', tabla: cat, id: String(id)}));
        const r = await j(`${API}?${qs.toString()}`);
  ...
  (async function boot(){
    ...
    await loadMeta();
    renderHead();
    if (IS_MULTI) await loadEmpresas();
    await loadStats();
    await loadList();
  })();
})();
