...
$map = [
  // slug => [tabla, cols_editables, cols_busqueda (para list), thead, form_def]
  'computadoras' => [
...
if (!isset($map[$tabla]) && $action !== 'stats' && $action !== 'meta') {
...
try {
  switch ($action) {
    case 'meta': {
      // Devuelve thead + form def para construir UI
      $out = [];
    ...
    case 'save': {
      ...
      if ($id > 0) {
        // update (solo si pertenece a la empresa)
        ...
        $setCols = [];
        $types=''; $pars=[];
        foreach ($def['cols'] as $c) { $setCols[] = "$c=?"; $types.='s'; $pars[] = $data[$c]; }
        // tipos para 'activo' como int
        $types = str_repeat('s', count($def['cols']));
        if (in_array('activo',$def['cols'], true)) {
          $idx = array_search('activo', $def['cols'], true);
          if ($idx !== false) { $types[$idx] = 'i'; } // (truco fino opcional)
        }
        // más robusto: convertir ints
        $types = '';
        foreach ($def['cols'] as $c) $types .= ($c==='activo' ? 'i' : 's');

        $sql = "UPDATE $table SET ".implode(',',$setCols)." WHERE id=? AND id_empresa=? LIMIT 1";
        $st = $mysqli->prepare($sql);
        $types .= 'ii'; $pars[]=$id; $pars[]=$empresaId;
        $st->bind_param($types, ...$pars);
        $st->execute();
        jok(['id'=>$id]);
      } else {
