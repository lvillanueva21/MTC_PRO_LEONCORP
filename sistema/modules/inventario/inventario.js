...
(function () {
  ...
  var bienImgCamFile = $('#bienImgCamFile');
  var btnImgPick = $('#btnImgPick');
  var btnImgCam  = $('#btnImgCam');
...
syncCatChips();
  ...
  function serieLine(row) {
  var s = (row && row.serie != null) ? String(row.serie) : '';
  s = s.trim();
  return s ? s : '-';
}

function brandModelLine(row) {
  var m = (row && row.marca != null) ? String(row.marca) : '';
  var mo = (row && row.modelo != null) ? String(row.modelo) : '';
  m = m.trim();
  mo = mo.trim();
  var bm = [m, mo].filter(Boolean).join(' ');
  return bm;
}

function nameWithBrandModelHtml(row) {
  var n = (row && row.nombre != null) ? String(row.nombre) : '';
  n = n.trim();
  var bm = brandModelLine(row);

  if (bm) {
    // Sin text-muted: hereda el color/estilo del contenedor (igual que el nombre)
    return esc(n) + ' <span class="ml-2">' + esc(bm) + '</span>';
  }
  return esc(n);
}
    ...
    tb.innerHTML = rows.map(function (x) {
      ...
      return (
        '<tr class="invx-main-row' + openCls + '" data-id="' + id + '">' +
          ...
          '<td>' +
            '<div class="font-weight-bold">' + nameWithBrandModelHtml(x) + '</div>' +
        ...
        '<div class="invx-card" data-id="' + id + '">' +
          ...
          '<div class="invx-card-body">' +
            '<div class="invx-card-main">' +
             ...
             '<div class="name">' + nameWithBrandModelHtml(x) + '</div>' +
...
function paintPager() {
  ...
  var cur = parseInt(state.page || 1, 10);
  if (!cur || cur < 1) cur = 1;
  ...
  // Ventana: max 3 a la izquierda y 3 a la derecha
  var win = 3;
  var start = cur - win;
  var end   = cur + win;
  ...
  // Si hay páginas antes de la ventana, mostrar 1 + (… si hay salto real)
  if (start > 1) {
    out.push(li(1, '1', (cur === 1) ? 'active' : ''));
    if (start > 2) out.push(dots());
  }

  // Páginas de la ventana (incluye 1 y última cuando corresponda)
  for (var p = start; p <= end; p++) {
    if (p === 1 && start > 1) continue;      // evita duplicar 1 si ya se puso arriba
    if (p === pages && end < pages) continue; // evita duplicar última si se pondrá abajo
    out.push(li(p, String(p), (p === cur) ? 'active' : ''));
  }

  // Si hay páginas después de la ventana, mostrar (… si hay salto real) + última
  if (end < pages) {
...
if (btnCatClear) {
  ...
  async function onBienImageSelected(file) {
  imgUploadErr = null;
...
// Botones
if (btnImgPick && bienImgFile) {
  btnImgPick.addEventListener('click', function () {
    try { bienImgFile.click(); } catch (e) {}
  });
}
if (btnImgCam && bienImgCamFile) {
  btnImgCam.addEventListener('click', function () {
    try { bienImgCamFile.click(); } catch (e) {}
  });
}

// Cambios de inputs
if (bienImgFile) {
  bienImgFile.addEventListener('change', function () {
    var file = (bienImgFile.files && bienImgFile.files[0]) ? bienImgFile.files[0] : null;
    onBienImageSelected(file);
  });
}
if (bienImgCamFile) {
  bienImgCamFile.addEventListener('change', function () {
    var file = (bienImgCamFile.files && bienImgCamFile.files[0]) ? bienImgCamFile.files[0] : null;
    onBienImageSelected(file);
  });
}
