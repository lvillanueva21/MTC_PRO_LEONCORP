...
try {
  ...
  if ($action === 'list') {
    ...
    $per    = max(1, min(10, (int)($_GET['per'] ?? 10)));
    $q      = norm_s($_GET['q'] ?? '');
    ...
    $sql = "
      SELECT
        b.*,
        CONCAT('E', b.id_empresa, '-', LPAD(b.id,6,'0')) AS codigo_inv,
  ...
  if ($action === 'get') {
    ...
    $st = $mysqli->prepare("SELECT *, CONCAT('E', id_empresa, '-', LPAD(id,6,'0')) AS codigo_inv
 FROM inv_bienes WHERE id=? AND id_empresa=? LIMIT 1");
  ...
  if ($action === 'cat_add') {
    ...
    // Intentar insertar. Si ya existe (UK), devolver el existente sin romper.
    ...
    try {
      ...
      $errNo = (int)($mysqli->errno ?: ($st->errno ?? 0));
      if ($errNo === 1062) { $dup = true; }
      else { throw $e; }
    }

    if ($insertOk) {
      jok(['id'=>(int)$mysqli->insert_id,'nombre'=>$nombre,'exists'=>0]);
    }

    // Duplicado: buscar ID existente
    $st2 = $mysqli->prepare("SELECT id,nombre FROM inv_categorias WHERE id_empresa=? AND nombre=? LIMIT 1");
    $st2->bind_param('is', $empresaId, $nombre);
    $st2->execute();
    $r = $st2->get_result()->fetch_assoc();

    if ($r && !empty($r['id'])) {
      jok(['id'=>(int)$r['id'],'nombre'=>(string)$r['nombre'],'exists'=>1]);
    }

    // Fallback raro
    jerror(409,'La categoría ya existe');
  }
  ...
  if ($action === 'ubic_add') {
    ...
    // Duplicado: buscar ID existente
    $st2 = $mysqli->prepare("SELECT id,nombre FROM inv_ubicaciones WHERE id_empresa=? AND nombre=? LIMIT 1");
    $st2->bind_param('is', $empresaId, $nombre);
    $st2->execute();
    $r = $st2->get_result()->fetch_assoc();
    ...
    // Fallback raro
    jerror(409,'La ubicación ya existe');
  }
