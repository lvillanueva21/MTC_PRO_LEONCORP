...
export function init(slot, apiUrl) {
  ...
  const cursosApi = slot.getAttribute('data-cursos-api') || 'cursos/api.php';
  // --- Normalizador de URLs para fotos de perfil guardadas
// En BD viene algo como 'almacen/img_perfil/archivo.ext'.
// Aquí le anteponemos el prefijo del proyecto ('/sistema' o '') según la URL actual.
const PROJECT_ROOT = (location.pathname.split('/modules/')[0] || '');
function fotoUrl(relPath) {
  if (!relPath) return '';
  const clean = String(relPath).replace(/^\/+/, ''); // quitar barra inicial si la hubiera
