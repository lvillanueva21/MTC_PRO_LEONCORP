...
async function j(url, opts = {}) {
  ...
  try{ await j(apiUrl,{method:'POST',body:fd}); showOK('Mensual abierta.'); await recargarTodo(); }
  catch(err){ showERR(err.message||'No se pudo abrir mensual'); }
}
async function cerrarMensual(empresaId){
  ...
  fd.append('empresa_id', String(emp));
  try{ await j(apiUrl,{method:'POST',body:fd}); showOK('Mensual cerrada.'); await recargarTodo(); }
  catch(err){ showERR(err.message||'No se pudo cerrar mensual'); }
}
async function abrirDiaria(fecha, empresaId){
  ...
  try{ await j(apiUrl,{method:'POST',body:fd}); showOK('Diaria abierta.'); await recargarTodo(); }
  catch(err){ showERR(err.message||'No se pudo abrir diaria'); }
}
async function cerrarDiaria(empresaId){
  ...
  fd.append('empresa_id', String(emp));
  try{ await j(apiUrl,{method:'POST',body:fd}); showOK('Diaria cerrada.'); await recargarTodo(); }
  catch(err){ showERR(err.message||'No se pudo cerrar diaria'); }
}


    // === NUEVOS: crear (no reabrir) ===
  async function crearMensual(ymStr){
    ...
    try{ await j(apiUrl,{method:'POST',body:fd}); showOK('Mensual creada y abierta.'); await recargarTodo(); }
    catch(err){ showERR(err.message||'No se pudo crear mensual'); }
  }
  ...
  async function crearDiaria(fecha){
    ...
    fd.append('empresa_id',String(FX.empresa_id));
    fd.append('fecha',fecha);
    try{ await j(apiUrl,{method:'POST',body:fd}); showOK('Diaria creada y abierta.'); await recargarTodo(); }
    catch(err){ showERR(err.message||'No se pudo crear diaria'); }
  }
