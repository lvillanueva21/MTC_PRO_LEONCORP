...
try{
  ...
  switch ($action){
    ...
    /* ---------- crear (nuevas acciones) ---------- */
        case 'crear_mensual': {
      // Apertura rápida (1.b). Mejora: idempotente; si ya está abierta, NO-OP sin tocar abierto_en.
      $a    = actor();
      ...
      try{
        ...
        // Objetivo
        $q1 = $db->prepare("SELECT id, estado FROM mod_caja_mensual WHERE id_empresa=? AND anio=? AND mes=? LIMIT 1 FOR UPDATE");
        ...
        if ($row){
          ...
          if ($row['estado']==='abierta'){
            // No tocar timestamps
            $db->commit();
            jok(['id'=>$cm_id,'noop'=>true,'msg'=>'Ya estaba abierta']);
          } else {
            // Reapertura real
            ...
            audit_counted($db,'abrir_mensual',$emp,$cm_id,null,sprintf('%04d-%02d',$anio,$mes).' | Reapertura');
          }
        } else {
          ...
          audit_counted($db,'abrir_mensual',$emp,$cm_id,null,sprintf('%04d-%02d',$anio,$mes).' | Creación');
        }
    ...
    case 'crear_diaria': {
      // Apertura rápida de DIARIA. Mejora: idempotente; evita tocar abierto_en si ya está abierta.
      $a     = actor();
      $emp   = (int)($_POST['empresa_id'] ?? 0);
      ...
      try{
        ...
        $qd = $db->prepare("SELECT id, estado FROM mod_caja_diaria WHERE id_empresa=? AND fecha=? LIMIT 1 FOR UPDATE");
        ...
        if ($row){
          ...
          if ($row['estado']==='abierta'){
            $db->commit();
            jok(['id'=>$cd_id,'noop'=>true,'msg'=>'Ya estaba abierta']);
          } else {
            ...
            audit_counted($db,'abrir_diaria',$emp,$id_cm,$cd_id,$fecha.' | Reapertura');
          }
        } else {
          ...
          audit_counted($db,'abrir_diaria',$emp,$id_cm,$cd_id,$fecha.' | Creación');
        }
    ...
    /* ---------- acciones caja ---------- */
        case 'abrir_mensual': {
      // Acción desde la lista: abrir período indicado.
      // Mejora: idempotente y sin tocar abierto_en si ya está abierta.
      $a    = actor();
      $emp  = (int)($_POST['empresa_id'] ?? 0);
      $anio = (int)($_POST['anio'] ?? 0);
      $mes  = (int)($_POST['mes'] ?? 0);
      ...
      $db->begin_transaction();
      try{
        // Bloquea objetivo primero
        $st = $db->prepare("SELECT id, estado FROM mod_caja_mensual WHERE id_empresa=? AND anio=? AND mes=? LIMIT 1 FOR UPDATE");
        $st->bind_param('iii',$emp,$anio,$mes); $st->execute();
        $target = $st->get_result()->fetch_assoc(); // puede no existir

        // Bloquea la mensual actualmente abierta (si la hay)
        ...
        // Si la abierta actual ES el objetivo y ya está abierta => NO-OP
        if ($prevM && $target && (int)$prevM['id'] === (int)$target['id'] && $target['estado']==='abierta') {
          $db->commit();
          jok(['id'=>(int)$target['id'],'noop'=>true,'msg'=>'Ya estaba abierta']);
        }

        // Si hay diaria abierta y vamos a cambiar de mensual => ciérrala
        if ($prevM && (!$target || (int)$prevM['id'] !== (int)$target['id'])) {
          $d = $db->prepare("SELECT id FROM mod_caja_diaria WHERE id_empresa=? AND estado='abierta' FOR UPDATE");
          $d->bind_param('i',$emp); $d->execute();
          ...
          // Cierra mensual previa
          $u2 = $db->prepare("UPDATE mod_caja_mensual SET estado='cerrada', cerrado_por=?, cerrado_en=NOW() WHERE id=?");
          $u2->bind_param('ii',$a['id'],$prevM['id']); $u2->execute();
          audit_counted($db,'cerrar_mensual',$emp,(int)$prevM['id'],null,'Cierre por apertura de otra mensual');
        }

        $cm_id = null;
        if ($target){
          if ($target['estado']==='cerrada'){
            // Reapertura real
            $up = $db->prepare("UPDATE mod_caja_mensual
                                SET estado='abierta', abierto_por=?, abierto_en=NOW(),
                                    cerrado_por=NULL, cerrado_en=NULL
                                WHERE id=?");
            $up->bind_param('ii',$a['id'],$target['id']); $up->execute();
            $cm_id = (int)$target['id'];
            audit_counted($db,'abrir_mensual',$emp,$cm_id,null,sprintf('%04d-%02d',$anio,$mes).' | Reapertura');
          } else {
            // target existe y está abierta pero no era prevM (raro por constraint, pero por seguridad)
            $cm_id = (int)$target['id'];
          }
        } else {
          // Crear nueva del período
          $cod = gen_codigo_mensual($emp,$anio,$mes);
          $ins = $db->prepare("INSERT INTO mod_caja_mensual (id_empresa, anio, mes, codigo, estado, abierto_por)
                               VALUES (?,?,?,?, 'abierta', ?)");
          $ins->bind_param('iiisi',$emp,$anio,$mes,$cod,$a['id']); $ins->execute();
    ...
    case 'cerrar_mensual': {
      ...
      try{
        ...
        case 'abrir_diaria': {
      // Mejora: idempotente; si la fecha ya está abierta, NO-OP sin tocar abierto_en.
      $a = actor();
      $emp = (int)($_POST['empresa_id'] ?? 0);
      $fecha = trim($_POST['fecha'] ?? '');
      if ($emp<=0 || !preg_match('/^\d{4}-\d{2}-\d{2}$/',$fecha)) jerror(400,'Datos inválidos');
      ...
      try{
        ...
        // ¿Ya está abierta para la misma fecha?
        $q = $db->prepare("SELECT id, estado FROM mod_caja_diaria WHERE id_empresa=? AND fecha=? LIMIT 1 FOR UPDATE");
        $q->bind_param('is',$emp,$fecha); $q->execute();
        $row = $q->get_result()->fetch_assoc();

        // Diaria abierta actual (si hay) para potencial cierre
        $lkD = $db->prepare("SELECT id, fecha FROM mod_caja_diaria WHERE id_empresa=? AND estado='abierta' FOR UPDATE");
        $lkD->bind_param('i',$emp); $lkD->execute();
        $dprev = $lkD->get_result()->fetch_assoc();

        if ($row && $row['estado']==='abierta') {
          // Si además es la misma que está abierta actualmente => NO-OP
          if ($dprev && (int)$dprev['id'] === (int)$row['id']) {
            $db->commit();
            jok(['id'=>(int)$row['id'],'noop'=>true,'msg'=>'Ya estaba abierta']);
          }
        }

        // Si hay otra diaria abierta y la fecha es distinta, ciérrala
        if ($dprev && (!$row || (int)$dprev['id'] !== (int)$row['id'])) {
          $u = $db->prepare("UPDATE mod_caja_diaria SET estado='cerrada', cerrado_por=?, cerrado_en=NOW() WHERE id=?");
        ...
        if ($row){
          ...
          if ($row['estado']==='cerrada'){
            ...
            audit_counted($db,'abrir_diaria',$emp,(int)$mOpen['id'],$id,$fecha.' | Reapertura');
          }
          $cd_id = $id;
        } else {
          $cod = gen_codigo_diaria($emp,$fecha);
          ...
          $cd_id = (int)$db->insert_id;
          audit_counted($db,'abrir_diaria',$emp,(int)$mOpen['id'],$cd_id,$fecha.' | Creación');
        }
