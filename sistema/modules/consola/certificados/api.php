<?php
// modules/consola/certificados/api.php
require_once __DIR__ . '/../../../includes/conexion.php';
require_once __DIR__ . '/../gestion_archivos.php';
require_once __DIR__ . '/fs_assets.php';

header('Content-Type: application/json; charset=utf-8');
mysqli_report(MYSQLI_REPORT_ERROR | MYSQLI_REPORT_STRICT);

$db = db();
$db->set_charset('utf8mb4');

$A = $_POST['action'] ?? $_GET['action'] ?? '';

function jerror($code, $msg, $extra = []) {
  http_response_code($code);
  echo json_encode(['ok' => false, 'msg' => $msg] + $extra);
  exit;
}
function jok($arr = []) {
  echo json_encode(['ok' => true] + $arr);
  exit;
}

try {
  switch ($A) {
    // -------------------- COMBO EMPRESAS --------------------
    case 'empresas': {
      $rs = $db->query("SELECT id, nombre FROM mtp_empresas ORDER BY nombre");
      jok(['data' => $rs->fetch_all(MYSQLI_ASSOC)]);
    }

    case 'list': {
  $q       = trim($_GET['q'] ?? '');
  $empresa = (int)($_GET['empresa_id'] ?? 0);
  $page    = max(1,(int)($_GET['page'] ?? 1));
  $perPage = max(1,min(50,(int)($_GET['per_page'] ?? 5)));
  $offset  = ($page-1)*$perPage;

  $where=[]; $types=''; $pars=[];
  if ($q!==''){
    $like="%$q%";
    $where[]="(pc.nombre LIKE ? COLLATE utf8mb4_spanish_ci
               OR pc.resolucion LIKE ? COLLATE utf8mb4_spanish_ci
               OR e.nombre LIKE ? COLLATE utf8mb4_spanish_ci)";
    $types.='sss'; array_push($pars,$like,$like,$like);
  }
  if ($empresa>0){
    $where[]="pc.id_empresa = ?";
    $types.='i'; $pars[]=$empresa;
  }
  $W = $where ? ('WHERE '.implode(' AND ',$where)) : '';

  // total
  $sqlC = "SELECT COUNT(*) c
           FROM cq_plantillas_certificados pc
           JOIN mtp_empresas e ON e.id=pc.id_empresa
           $W";
  $stC = $db->prepare($sqlC); if($types) $stC->bind_param($types, ...$pars);
  $stC->execute(); $total=(int)$stC->get_result()->fetch_assoc()['c'];

  // data
  $sqlD="SELECT pc.id, pc.nombre, pc.paginas, pc.id_empresa, pc.representante,
                pc.ciudad, pc.resolucion, pc.fondo_path, pc.logo_path, pc.firma_path,
                pc.activo, pc.creado, e.nombre AS empresa
         FROM cq_plantillas_certificados pc
         JOIN mtp_empresas e ON e.id=pc.id_empresa
         $W
         ORDER BY pc.id DESC
         LIMIT ? OFFSET ?";
  $types2=$types.'ii'; $pars2=$pars; $pars2[]=$perPage; $pars2[]=$offset;
  $stD=$db->prepare($sqlD); $stD->bind_param($types2, ...$pars2); $stD->execute();
  $rows=$stD->get_result()->fetch_all(MYSQLI_ASSOC);

  jok(['data'=>$rows,'total'=>$total,'page'=>$page,'per_page'=>$perPage]);
}

    case 'get': {
      $id = (int)($_GET['id'] ?? 0);
      if ($id <= 0) jerror(400, 'ID inválido');

      $st = $db->prepare(
        "SELECT pc.id, pc.nombre, pc.paginas, pc.id_empresa, pc.representante, pc.ciudad, pc.resolucion,
                pc.fondo_path, pc.logo_path, pc.firma_path, pc.activo, pc.creado, pc.actualizado,
                e.nombre AS empresa
         FROM cq_plantillas_certificados pc
         JOIN mtp_empresas e ON e.id = pc.id_empresa
         WHERE pc.id = ?"
      );
      $st->bind_param('i', $id);
      $st->execute();
      $row = $st->get_result()->fetch_assoc();
      if (!$row) jerror(404, 'No encontrado');

      jok(['data' => $row]);
    }

    // -------------------- CREAR PLANTILLA --------------------
    case 'create': {
      $nombre        = trim($_POST['nombre'] ?? '');
      $paginas       = (int)($_POST['paginas'] ?? 0);
      $id_empresa    = (int)($_POST['id_empresa'] ?? 0);
      $representante = trim($_POST['representante'] ?? '');
      $ciudad        = trim($_POST['ciudad'] ?? '');
      $resolucion    = trim($_POST['resolucion'] ?? '');

      if ($nombre === '') jerror(400, 'El nombre es obligatorio');
      if ($paginas <= 0 || $paginas > 255) jerror(400, 'Páginas debe ser un número entre 1 y 255');
      if ($id_empresa <= 0) jerror(400, 'Debes seleccionar una empresa');

      // Fondo obligatorio
      if (empty($_FILES['fondo']) || ($_FILES['fondo']['error'] ?? UPLOAD_ERR_NO_FILE) === UPLOAD_ERR_NO_FILE) {
        jerror(400, 'El fondo de certificado es obligatorio');
      }

      // Valida tipos/tamaños (5MB máx)
      $checkImg = function(array $file) {
        if (($file['error'] ?? UPLOAD_ERR_OK) !== UPLOAD_ERR_OK) jerror(400, 'Error al subir imagen');
        if (($file['size'] ?? 0) > 5 * 1024 * 1024) jerror(400, 'La imagen excede 5MB');
        $fi = new finfo(FILEINFO_MIME_TYPE);
        $mime = $fi->file($file['tmp_name']);
        if (!in_array($mime, ['image/jpeg','image/png','image/webp'], true)) {
          jerror(400, 'Formato no permitido (PNG/JPG/WebP)');
        }
      };

      $checkImg($_FILES['fondo']);
      if (!empty($_FILES['logo']) && ($_FILES['logo']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE) $checkImg($_FILES['logo']);
      if (!empty($_FILES['firma']) && ($_FILES['firma']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE) $checkImg($_FILES['firma']);

      // Guardar imágenes primero (con entidad NULL; nombre estable por timestamp y random)
      $ruta_fondo = $ruta_logo = $ruta_firma = null;
      try {
        $gaF = ga_save_upload($db, $_FILES['fondo'], 'fondo_certificado', 'fondo-certificado', 'certificados', 'plantilla', null);
        $ruta_fondo = $gaF['ruta_relativa'] ?? null;

        if (!empty($_FILES['logo']) && ($_FILES['logo']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE) {
          $gaL = ga_save_upload($db, $_FILES['logo'], 'logo_certificado', 'logo-certificado', 'certificados', 'plantilla', null);
          $ruta_logo = $gaL['ruta_relativa'] ?? null;
        }
        if (!empty($_FILES['firma']) && ($_FILES['firma']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE) {
          $gaR = ga_save_upload($db, $_FILES['firma'], 'firma_representante', 'firma-representante', 'certificados', 'plantilla', null);
          $ruta_firma = $gaR['ruta_relativa'] ?? null;
        }
      } catch (Throwable $e) {
        jerror(500, 'No se pudieron guardar las imágenes', ['dev' => $e->getMessage()]);
      }

      if (!$ruta_fondo) jerror(500, 'No se obtuvo ruta del fondo');

      // Insert
      $db->begin_transaction();
      try {
        $sql = "INSERT INTO cq_plantillas_certificados
                  (nombre, paginas, id_empresa, representante, ciudad, resolucion,
                   fondo_path, logo_path, firma_path, activo)
                VALUES (?,?,?,?,?,?,?,?,?,1)";
        $st = $db->prepare($sql);
        // Nulos opcionales si vienen vacíos
        $rep = ($representante !== '') ? $representante : null;
        $ciu = ($ciudad !== '') ? $ciudad : null;
        $res = ($resolucion !== '') ? $resolucion : null;

        $st->bind_param(
          'siissssss',
          $nombre,        // s
          $paginas,       // i
          $id_empresa,    // i
          $rep,           // s (NULL OK)
          $ciu,           // s (NULL OK)
          $res,           // s (NULL OK)
          $ruta_fondo,    // s
          $ruta_logo,     // s (NULL OK)
          $ruta_firma     // s (NULL OK)
        );
        $st->execute();
        $pid = (int)$db->insert_id;

        $db->commit();
        jok(['id' => $pid]);
      } catch (mysqli_sql_exception $e) {
        $db->rollback();
        // Limpia archivos guardados si el insert falla
        if ($ruta_fondo) ga_mark_and_delete($db, $ruta_fondo, 'borrado');
        if ($ruta_logo)  ga_mark_and_delete($db, $ruta_logo,  'borrado');
        if ($ruta_firma) ga_mark_and_delete($db, $ruta_firma, 'borrado');

        if ((int)$e->getCode() === 1062) {
          jerror(409, 'Ya existe una plantilla con ese nombre para la empresa');
        }
        jerror(500, 'Error del servidor', ['dev' => $e->getMessage()]);
      }
    }

    case 'update': {
      $id           = (int)($_POST['id'] ?? 0);
      $nombre       = trim($_POST['nombre'] ?? '');
      $paginas      = max(1, (int)($_POST['paginas'] ?? 1));
      $id_empresa   = (int)($_POST['id_empresa'] ?? 0);
      $representante= trim($_POST['representante'] ?? '');
      $ciudad       = trim($_POST['ciudad'] ?? '');
      $resolucion   = trim($_POST['resolucion'] ?? '');

      if ($id <= 0) jerror(400, 'ID inválido');
      if ($nombre === '') jerror(400, 'El nombre es obligatorio');
      if ($id_empresa <= 0) jerror(400, 'Empresa requerida');

      // Traer paths previos (para borrar si se reemplazan)
      $stp = $db->prepare("SELECT fondo_path, logo_path, firma_path FROM cq_plantillas_certificados WHERE id=?");
      $stp->bind_param('i', $id);
      $stp->execute();
      $prev = $stp->get_result()->fetch_assoc();
      if (!$prev) jerror(404, 'No encontrado');

      $db->begin_transaction();
      try {
        // Actualizar datos base
        $st = $db->prepare("UPDATE cq_plantillas_certificados
                            SET nombre=?, paginas=?, id_empresa=?, representante=?, ciudad=?, resolucion=?
                            WHERE id=?");
        $st->bind_param('siisssi', $nombre, $paginas, $id_empresa, $representante, $ciudad, $resolucion, $id);
        $st->execute();

        // Reemplazo de imágenes (si se envían)
        // Fondo (obligatorio al crear; en update solo si adjuntan uno nuevo)
        if (!empty($_FILES['fondo']) && $_FILES['fondo']['error'] !== UPLOAD_ERR_NO_FILE) {
          if ($_FILES['fondo']['error'] !== UPLOAD_ERR_OK) jerror(400, 'Error al subir el fondo');
          if (($_FILES['fondo']['size'] ?? 0) > 10*1024*1024) jerror(400, 'El fondo excede 10MB');
          $ga = ga_save_upload($db, $_FILES['fondo'], 'fondo_certificado', 'fondo-certificado', 'certificados', 'plantilla', $id);
          $ruta_rel = $ga['ruta_relativa'] ?? '';
          if ($ruta_rel !== '') {
            $up = $db->prepare("UPDATE cq_plantillas_certificados SET fondo_path=? WHERE id=?");
            $up->bind_param('si', $ruta_rel, $id);
            $up->execute();
            if (!empty($prev['fondo_path']) && $prev['fondo_path'] !== $ruta_rel) {
              ga_mark_and_delete($db, $prev['fondo_path'], 'reemplazado');
            }
          }
        }
        // Logo (opcional)
        if (!empty($_FILES['logo']) && $_FILES['logo']['error'] !== UPLOAD_ERR_NO_FILE) {
          if ($_FILES['logo']['error'] !== UPLOAD_ERR_OK) jerror(400, 'Error al subir el logo');
          if (($_FILES['logo']['size'] ?? 0) > 5*1024*1024) jerror(400, 'El logo excede 5MB');
          $ga = ga_save_upload($db, $_FILES['logo'], 'logo_certificado', 'logo-certificado', 'certificados', 'plantilla', $id);
          $ruta_rel = $ga['ruta_relativa'] ?? '';
          if ($ruta_rel !== '') {
            $up = $db->prepare("UPDATE cq_plantillas_certificados SET logo_path=? WHERE id=?");
            $up->bind_param('si', $ruta_rel, $id);
            $up->execute();
            if (!empty($prev['logo_path']) && $prev['logo_path'] !== $ruta_rel) {
              ga_mark_and_delete($db, $prev['logo_path'], 'reemplazado');
            }
          }
        }
        // Firma (opcional)
        if (!empty($_FILES['firma']) && $_FILES['firma']['error'] !== UPLOAD_ERR_NO_FILE) {
          if ($_FILES['firma']['error'] !== UPLOAD_ERR_OK) jerror(400, 'Error al subir la firma');
          if (($_FILES['firma']['size'] ?? 0) > 5*1024*1024) jerror(400, 'La firma excede 5MB');
          $ga = ga_save_upload($db, $_FILES['firma'], 'firma_representante', 'firma-representante', 'certificados', 'plantilla', $id);
          $ruta_rel = $ga['ruta_relativa'] ?? '';
          if ($ruta_rel !== '') {
            $up = $db->prepare("UPDATE cq_plantillas_certificados SET firma_path=? WHERE id=?");
            $up->bind_param('si', $ruta_rel, $id);
            $up->execute();
            if (!empty($prev['firma_path']) && $prev['firma_path'] !== $ruta_rel) {
              ga_mark_and_delete($db, $prev['firma_path'], 'reemplazado');
            }
          }
        }

        $db->commit();
        jok(['id'=>$id]);
      } catch (Throwable $e) {
        $db->rollback();
        jerror(500, 'Error del servidor', ['dev'=>$e->getMessage()]);
      }
    }
    
        // -------------------- GUARDAR / LEER ELEMENTOS POR PLANTILLA --------------------
    case 'elements_get': {
      $id = (int)($_GET['id'] ?? 0);
      if ($id <= 0) jerror(400, 'ID inválido');

      // Verificar que la plantilla existe
      $st = $db->prepare("SELECT id FROM cq_plantillas_certificados WHERE id = ?");
      $st->bind_param('i', $id);
      $st->execute();
      $row = $st->get_result()->fetch_assoc();
      $st->close();
      if (!$row) {
        jerror(404, 'Plantilla no encontrada');
      }

      // Leer elementos actuales
      $st = $db->prepare("
        SELECT codigo_elemento
        FROM cq_plantillas_elementos
        WHERE id_plantilla_certificado = ?
        ORDER BY codigo_elemento
      ");
      $st->bind_param('i', $id);
      $st->execute();
      $res = $st->get_result();

      $elems = [];
      while ($r = $res->fetch_assoc()) {
        $elems[] = (string)$r['codigo_elemento'];
      }
      $st->close();

      jok(['data' => $elems]);
    }

    case 'elements_save': {
      $id = (int)($_POST['id'] ?? 0);
      if ($id <= 0) jerror(400, 'ID inválido');

      // Verificar que la plantilla existe
      $st = $db->prepare("SELECT id FROM cq_plantillas_certificados WHERE id = ?");
      $st->bind_param('i', $id);
      $st->execute();
      $row = $st->get_result()->fetch_assoc();
      $st->close();
      if (!$row) {
        jerror(404, 'Plantilla no encontrada');
      }

      // Lista de elementos permitidos (por seguridad)
      // Esta lista debe mantenerse en sincronía con ELEMENTS en gestion.js
      $allowed = [
        'plantilla_nombre',       // nombre de la plantilla o título principal
        'plantilla_resolucion',   // resolución / norma
        'nombre_completo',        // nombres + apellidos
        'curso',                  // nombre del curso
        'detalle_curso',          // detalle de horas / modalidad
        'codigo_certificado',     // código único del certificado
        'representante',          // nombre del representante
        'fecha_caducidad',        // fecha de caducidad / vigencia
        'ciudad_fecha_emision',   // ciudad, fecha de emisión
        'qr'                      // código QR (imagen)
      ];

      // Viene como JSON desde el JS
      $json = $_POST['elements'] ?? '[]';
      $elements = json_decode($json, true);
      if (!is_array($elements)) {
        $elements = [];
      }

      // Normalizar y filtrar
      $clean = [];
      foreach ($elements as $code) {
        $code = trim((string)$code);
        if ($code === '') continue;
        if (!in_array($code, $allowed, true)) continue;
        $clean[] = $code;
      }
      $clean = array_values(array_unique($clean));

      $db->begin_transaction();
      try {
        // Borrar elementos anteriores de esa plantilla
        $stDel = $db->prepare("
          DELETE FROM cq_plantillas_elementos
          WHERE id_plantilla_certificado = ?
        ");
        $stDel->bind_param('i', $id);
        $stDel->execute();
        $stDel->close();

        // Insertar solo los seleccionados
        if ($clean) {
          $stIns = $db->prepare("
            INSERT INTO cq_plantillas_elementos (id_plantilla_certificado, codigo_elemento)
            VALUES (?, ?)
          ");
          foreach ($clean as $code) {
            $stIns->bind_param('is', $id, $code);
            $stIns->execute();
          }
          $stIns->close();
        }

        $db->commit();
        jok(['msg' => 'Elementos guardados']);
      } catch (Throwable $e) {
        $db->rollback();
        jerror(500, 'Error al guardar elementos', ['dev' => $e->getMessage()]);
      }
    }
            case 'layout_get': {
      $id = (int)($_GET['id'] ?? 0);
      if ($id <= 0) jerror(400, 'ID inválido');

      // Verificar que la plantilla existe
      $st = $db->prepare("SELECT id FROM cq_plantillas_certificados WHERE id = ?");
      $st->bind_param('i', $id);
      $st->execute();
      $row = $st->get_result()->fetch_assoc();
      $st->close();
      if (!$row) {
        jerror(404, 'Plantilla no encontrada');
      }

      // Valores por defecto (coherentes con la vista previa JS)
      $logo     = ['x' => 50.0, 'y' => 15.0, 'w' => 30.0];
      $firma    = ['x' => 80.0, 'y' => 80.0, 'w' => 25.0];
      $qr       = ['x' => 85.0, 'y' => 20.0, 'w' => 18.0];
      $elements = [];

$st2 = $db->prepare("
  SELECT codigo_elemento, pos_x, pos_y, ancho, ejemplo_texto,
         font_size, font_bold, font_align, font_family, font_color
  FROM cq_plantillas_posiciones
  WHERE id_plantilla_certificado = ? AND pagina = 1
");

      $st2->bind_param('i', $id);
      $st2->execute();
      $rs = $st2->get_result();

      while ($r = $rs->fetch_assoc()) {
        $code = (string)$r['codigo_elemento'];
        $x = (float)$r['pos_x'];
        $y = (float)$r['pos_y'];
        $w = (float)$r['ancho'];
        $t = isset($r['ejemplo_texto']) ? (string)$r['ejemplo_texto'] : '';

        $fs    = isset($r['font_size'])  ? (int)$r['font_size']  : 0;
        $bold  = isset($r['font_bold'])  ? ((int)$r['font_bold'] ? 1 : 0) : 0;
        $align = isset($r['font_align']) ? (string)$r['font_align'] : '';
        $font  = isset($r['font_family'])? (string)$r['font_family'] : '';
        $color = isset($r['font_color']) ? (string)$r['font_color'] : '';

        if ($code === 'logo') {
          $logo = ['x' => $x, 'y' => $y, 'w' => $w ?: $logo['w']];
        } elseif ($code === 'firma') {
          $firma = ['x' => $x, 'y' => $y, 'w' => $w ?: $firma['w']];
        } elseif ($code === 'qr') {
          $qr = ['x' => $x, 'y' => $y, 'w' => $w ?: $qr['w']];
        } else {
      $elements[$code] = [
        'x'          => $x,
        'y'          => $y,
        'w'          => $w,
        'texto'      => $t,
        'font_size'  => $fs,
        'bold'       => $bold,
        'align'      => $align,
        'font_family'=> $font,
        'font_color' => $color
      ];
    }
      }
      $st2->close();

      jok([
        'data' => [
          'logo'     => $logo,
          'firma'    => $firma,
          'qr'       => $qr,
          'elements' => $elements
        ]
      ]);
    }

    case 'layout_save': {
      $id = (int)($_POST['id'] ?? 0);
      if ($id <= 0) jerror(400, 'ID inválido');

      // Verificar plantilla
      $st = $db->prepare("SELECT id FROM cq_plantillas_certificados WHERE id = ?");
      $st->bind_param('i', $id);
      $st->execute();
      $row = $st->get_result()->fetch_assoc();
      $st->close();
      if (!$row) {
        jerror(404, 'Plantilla no encontrada');
      }

      $json = $_POST['layout'] ?? '{}';
      $data = json_decode($json, true);
      if (!is_array($data)) $data = [];

      // Códigos de elementos de texto permitidos (no incluye logo, firma ni qr)
      $allowedText = [
        'plantilla_nombre',
        'plantilla_resolucion',
        'nombre_completo',
        'curso',
        'detalle_curso',
        'codigo_certificado',
        'representante',
        'fecha_caducidad',
        'ciudad_fecha_emision'
      ];

      $rows = [];

      $clamp = function($v, $min, $max) {
        $v = (float)$v;
        if ($v < $min) $v = $min;
        if ($v > $max) $v = $max;
        return $v;
      };

            // Logo
      if (isset($data['logo']) && is_array($data['logo'])) {
        $p = $data['logo'];
        $rows[] = [
          'code'        => 'logo',
          'x'           => $clamp($p['x'] ?? 50, 0, 100),
          'y'           => $clamp($p['y'] ?? 15, 0, 100),
          'w'           => $clamp($p['w'] ?? 30, 5, 90),
          'texto'       => null,
          'font_size'   => 0,
          'font_bold'   => 0,
          'font_align'  => null,
          'font_family' => null,
          'font_color'  => null
        ];
      }

      // Firma
      if (isset($data['firma']) && is_array($data['firma'])) {
        $p = $data['firma'];
        $rows[] = [
          'code'        => 'firma',
          'x'           => $clamp($p['x'] ?? 80, 0, 100),
          'y'           => $clamp($p['y'] ?? 80, 0, 100),
          'w'           => $clamp($p['w'] ?? 25, 5, 90),
          'texto'       => null,
          'font_size'   => 0,
          'font_bold'   => 0,
          'font_align'  => null,
          'font_family' => null,
          'font_color'  => null
        ];
      }

      // QR (imagen)
      if (isset($data['qr']) && is_array($data['qr'])) {
        $p = $data['qr'];
        $rows[] = [
          'code'        => 'qr',
          'x'           => $clamp($p['x'] ?? 85, 0, 100),
          'y'           => $clamp($p['y'] ?? 20, 0, 100),
          'w'           => $clamp($p['w'] ?? 18, 5, 90),
          'texto'       => null,
          'font_size'   => 0,
          'font_bold'   => 0,
          'font_align'  => null,
          'font_family' => null,
          'font_color'  => null
        ];
      }

      // Otros elementos (posición, ancho, texto de ejemplo y estilo)
      if (isset($data['elements']) && is_array($data['elements'])) {
        foreach ($data['elements'] as $code => $p) {
          $code = (string)$code;
          if (!in_array($code, $allowedText, true)) continue;
          if (!is_array($p)) continue;

          $x = $clamp($p['x'] ?? 50, 0, 100);
          $y = $clamp($p['y'] ?? 50, 0, 100);
          $w = $clamp($p['w'] ?? 40, 5, 90);

          // Tamaño de fuente (porcentaje relativo, 50–200)
          $fs = isset($p['size']) ? (int)$p['size'] : 100;
          if ($fs < 50)  $fs = 50;
          if ($fs > 200) $fs = 200;

          // Negrita
          $bold = !empty($p['bold']) ? 1 : 0;

          // Alineación
          $align = strtoupper(substr((string)($p['align'] ?? 'C'), 0, 1));
          if (!in_array($align, ['L','C','R','J'], true)) {
            $align = 'C';
          }

                    // Fuente
          $font = (string)($p['font'] ?? '');
          if (!in_array($font, ['helvetica','times','courier'], true)) {
            $font = '';
          }

          // Color (hex #RRGGBB)
          $color = null;
          if (!empty($p['color'])) {
            $c = strtoupper(trim((string)$p['color']));
            if ($c[0] !== '#') {
              $c = '#'.$c;
            }
            if (preg_match('/^#[0-9A-F]{6}$/', $c)) {
              $color = $c;
            }
          }

          $rows[] = [
            'code'        => $code,
            'x'           => $x,
            'y'           => $y,
            'w'           => $w,
            'texto'       => isset($p['texto']) ? (string)$p['texto'] : null,
            'font_size'   => $fs,
            'font_bold'   => $bold,
            'font_align'  => $align,
            'font_family' => $font !== '' ? $font : null,
            'font_color'  => $color
          ];
        }
      }

      $db->begin_transaction();
      try {
        // Limpiar posiciones previas
        $stDel = $db->prepare("DELETE FROM cq_plantillas_posiciones WHERE id_plantilla_certificado = ?");
        $stDel->bind_param('i', $id);
        $stDel->execute();
        $stDel->close();

                if ($rows) {
          $stIns = $db->prepare("
            INSERT INTO cq_plantillas_posiciones
              (id_plantilla_certificado, codigo_elemento, pagina,
               pos_x, pos_y, ancho, ejemplo_texto,
               font_size, font_bold, font_align, font_family, font_color)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
          ");
          foreach ($rows as $r) {
            $code       = $r['code'];
            $pagina     = 1;
            $x          = $r['x'];
            $y          = $r['y'];
            $w          = $r['w'];
            $texto      = array_key_exists('texto', $r) ? $r['texto'] : null;
            $fontSize   = (int)($r['font_size'] ?? 0);
            $fontBold   = (int)($r['font_bold'] ?? 0);
            $fontAlign  = isset($r['font_align']) ? (string)$r['font_align'] : null;
            $fontFamily = isset($r['font_family']) ? (string)$r['font_family'] : null;
            $fontColor  = isset($r['font_color']) ? (string)$r['font_color'] : null;

            $stIns->bind_param(
              'isidddsiisss',
              $id,
              $code,
              $pagina,
              $x,
              $y,
              $w,
              $texto,
              $fontSize,
              $fontBold,
              $fontAlign,
              $fontFamily,
              $fontColor
            );
            $stIns->execute();
          }
          $stIns->close();
        }

        $db->commit();
        jok(['msg' => 'Posiciones guardadas']);
      } catch (Throwable $e) {
        $db->rollback();
        jerror(500, 'Error al guardar posiciones', ['dev' => $e->getMessage()]);
      }
    }
    // -------------------- COPIAR PLANTILLA (CLONAR) --------------------
    case 'copy': {
      $source_id        = (int)($_POST['source_id'] ?? 0);
      $target_empresa   = (int)($_POST['target_empresa_id'] ?? 0);
      $nombre_destino   = trim($_POST['nombre'] ?? '');
      $copy_assets      = (int)($_POST['copy_assets'] ?? 1) ? 1 : 0;

      if ($source_id <= 0) jerror(400, 'Plantilla origen inválida');
      if ($target_empresa <= 0) jerror(400, 'Empresa destino inválida');

      // Verificar empresa destino
      $stE = $db->prepare("SELECT id FROM mtp_empresas WHERE id=?");
      $stE->bind_param('i', $target_empresa);
      $stE->execute();
      $okEmp = $stE->get_result()->fetch_assoc();
      $stE->close();
      if (!$okEmp) jerror(404, 'Empresa destino no existe');

      // Obtener plantilla origen
      $stS = $db->prepare("
        SELECT id, nombre, paginas, id_empresa, representante, ciudad, resolucion,
               fondo_path, logo_path, firma_path, activo
        FROM cq_plantillas_certificados
        WHERE id=?
      ");
      $stS->bind_param('i', $source_id);
      $stS->execute();
      $src = $stS->get_result()->fetch_assoc();
      $stS->close();
      if (!$src) jerror(404, 'Plantilla origen no encontrada');

      // Resolver nombre destino
      $baseName = $nombre_destino !== '' ? $nombre_destino : ($src['nombre'].' (Copia)');
      $finalName = $baseName;

      // Si el usuario escribió nombre explícito, exigimos que sea único (no autoincrementamos)
      if ($nombre_destino !== '') {
        $stChk = $db->prepare("SELECT 1 FROM cq_plantillas_certificados WHERE id_empresa=? AND nombre=? LIMIT 1");
        $stChk->bind_param('is', $target_empresa, $finalName);
        $stChk->execute();
        $exists = $stChk->get_result()->fetch_assoc();
        $stChk->close();
        if ($exists) jerror(409, 'Ya existe una plantilla con ese nombre en la empresa destino');
      } else {
        // Auto-sufijo seguro si hay choque
        $stChk = $db->prepare("SELECT 1 FROM cq_plantillas_certificados WHERE id_empresa=? AND nombre=? LIMIT 1");
        for ($i = 0; $i < 30; $i++) {
          $try = ($i === 0) ? $baseName : ($baseName.' '.($i+1));
          $stChk->bind_param('is', $target_empresa, $try);
          $stChk->execute();
          $exists = $stChk->get_result()->fetch_assoc();
          if (!$exists) { $finalName = $try; break; }
        }
        $stChk->close();
      }

      // Copiar assets físicamente (recomendado)
      $ROOT = realpath(__DIR__ . '/../../..');
      if (!$ROOT) jerror(500, 'No se pudo resolver la raíz del proyecto');

      $new_fondo = $src['fondo_path'];
      $new_logo  = $src['logo_path'];
      $new_firma = $src['firma_path'];

      $createdFiles = []; // para cleanup si algo falla

      if ($copy_assets) {
        $c1 = pcx_copy_asset($ROOT, $src['fondo_path'], 'fondo_certificado');
        if (!$c1['ok'] || empty($c1['rel'])) jerror(500, 'No se pudo copiar el fondo', ['dev'=>$c1['msg'] ?? '']);
        $new_fondo = $c1['rel'];
        $createdFiles[] = $new_fondo;

        if (!empty($src['logo_path'])) {
          $c2 = pcx_copy_asset($ROOT, $src['logo_path'], 'logo_certificado');
          if (!$c2['ok']) {
            // cleanup parcial
            foreach ($createdFiles as $p) { pcx_delete_asset($ROOT, $p); }
            jerror(500, 'No se pudo copiar el logo', ['dev'=>$c2['msg'] ?? '']);
          }
          $new_logo = $c2['rel'];
          if (!empty($new_logo)) $createdFiles[] = $new_logo;
        } else {
          $new_logo = null;
        }

        if (!empty($src['firma_path'])) {
          $c3 = pcx_copy_asset($ROOT, $src['firma_path'], 'firma_representante');
          if (!$c3['ok']) {
            foreach ($createdFiles as $p) { pcx_delete_asset($ROOT, $p); }
            jerror(500, 'No se pudo copiar la firma', ['dev'=>$c3['msg'] ?? '']);
          }
          $new_firma = $c3['rel'];
          if (!empty($new_firma)) $createdFiles[] = $new_firma;
        } else {
          $new_firma = null;
        }
      }

      $db->begin_transaction();
      try {
        // Insert cabecera
        $sql = "INSERT INTO cq_plantillas_certificados
                  (nombre, paginas, id_empresa, representante, ciudad, resolucion,
                   fondo_path, logo_path, firma_path, activo)
                VALUES (?,?,?,?,?,?,?,?,?,?)";
        $stI = $db->prepare($sql);

        $paginas = (int)$src['paginas'];
        if ($paginas <= 0) $paginas = 1;

        $rep = $src['representante'] !== null && trim((string)$src['representante']) !== '' ? $src['representante'] : null;
        $ciu = $src['ciudad']        !== null && trim((string)$src['ciudad'])        !== '' ? $src['ciudad']        : null;
        $res = $src['resolucion']    !== null && trim((string)$src['resolucion'])    !== '' ? $src['resolucion']    : null;

        $logo  = ($new_logo  !== null && trim((string)$new_logo)  !== '') ? $new_logo  : null;
        $firma = ($new_firma !== null && trim((string)$new_firma) !== '') ? $new_firma : null;
        $activo = (int)$src['activo'] ? 1 : 1; // por defecto activa al copiar (si quieres respetar origen, cambia a (int)$src['activo'])

        $stI->bind_param(
          'siissssssi',
          $finalName,
          $paginas,
          $target_empresa,
          $rep,
          $ciu,
          $res,
          $new_fondo,
          $logo,
          $firma,
          $activo
        );
        $stI->execute();
        $new_id = (int)$db->insert_id;
        $stI->close();

        // Copiar elementos visibles
        $stE2 = $db->prepare("
          INSERT INTO cq_plantillas_elementos (id_plantilla_certificado, codigo_elemento)
          SELECT ?, codigo_elemento
          FROM cq_plantillas_elementos
          WHERE id_plantilla_certificado = ?
        ");
        $stE2->bind_param('ii', $new_id, $source_id);
        $stE2->execute();
        $stE2->close();

        // Copiar posiciones/estilos (todas las páginas)
        $stP = $db->prepare("
          INSERT INTO cq_plantillas_posiciones
            (id_plantilla_certificado, codigo_elemento, pagina,
             pos_x, pos_y, ancho, ejemplo_texto,
             font_size, font_bold, font_align, font_family, font_color)
          SELECT
            ?, codigo_elemento, pagina,
            pos_x, pos_y, ancho, ejemplo_texto,
            font_size, font_bold, font_align, font_family, font_color
          FROM cq_plantillas_posiciones
          WHERE id_plantilla_certificado = ?
        ");
        $stP->bind_param('ii', $new_id, $source_id);
        $stP->execute();
        $stP->close();

        $db->commit();
        jok(['id'=>$new_id, 'nombre'=>$finalName]);
      } catch (Throwable $e) {
        $db->rollback();
        // cleanup archivos copiados si falló la BD
        foreach ($createdFiles as $p) { pcx_delete_asset($ROOT, $p); }
        jerror(500, 'Error al copiar plantilla', ['dev'=>$e->getMessage()]);
      }
    }

    // -------------------- DESACTIVAR PLANTILLA (SOFT DELETE) --------------------
    case 'deactivate': {
      $id = (int)($_POST['id'] ?? 0);
      if ($id <= 0) jerror(400, 'ID inválido');

      $st = $db->prepare("UPDATE cq_plantillas_certificados SET activo=0 WHERE id=?");
      $st->bind_param('i', $id);
      $st->execute();
      $aff = $st->affected_rows;
      $st->close();

      if ($aff <= 0) jerror(404, 'Plantilla no encontrada o ya estaba desactivada');
      jok(['msg'=>'Plantilla desactivada']);
    }

    // -------------------- ELIMINAR PLANTILLA DEFINITIVAMENTE (BD + ARCHIVOS) --------------------
    case 'delete': {
      $id = (int)($_POST['id'] ?? 0);
      if ($id <= 0) jerror(400, 'ID inválido');

      // Traer paths antes de borrar
      $st = $db->prepare("SELECT fondo_path, logo_path, firma_path FROM cq_plantillas_certificados WHERE id=?");
      $st->bind_param('i', $id);
      $st->execute();
      $row = $st->get_result()->fetch_assoc();
      $st->close();
      if (!$row) jerror(404, 'Plantilla no encontrada');

      $fondo = (string)($row['fondo_path'] ?? '');
      $logo  = (string)($row['logo_path'] ?? '');
      $firma = (string)($row['firma_path'] ?? '');

      $db->begin_transaction();
      try {
        $stD = $db->prepare("DELETE FROM cq_plantillas_certificados WHERE id=?");
        $stD->bind_param('i', $id);
        $stD->execute();
        $aff = $stD->affected_rows;
        $stD->close();

        if ($aff <= 0) {
          $db->rollback();
          jerror(404, 'Plantilla no encontrada');
        }

        $db->commit();
      } catch (Throwable $e) {
        $db->rollback();
        jerror(500, 'Error al eliminar plantilla', ['dev'=>$e->getMessage()]);
      }

      // Borrar archivos físicos (solo dentro de almacen/)
      $ROOT = realpath(__DIR__ . '/../../..');
      if (!$ROOT) {
        jok(['msg'=>'Plantilla eliminada. No se pudo resolver raíz para borrar archivos.']);
      }

      $deleted = [];
      if ($fondo !== '' && pcx_delete_asset($ROOT, $fondo)) $deleted[] = $fondo;
      if ($logo  !== '' && pcx_delete_asset($ROOT, $logo))  $deleted[] = $logo;
      if ($firma !== '' && pcx_delete_asset($ROOT, $firma)) $deleted[] = $firma;

      jok(['msg'=>'Plantilla eliminada definitivamente', 'deleted'=>$deleted]);
    }
    default:
      jerror(400, 'Acción no válida');
  }
} catch (mysqli_sql_exception $e) {
  if ($db->errno) { @$db->rollback(); }
  jerror(500, 'Error de base de datos', ['dev' => $e->getMessage()]);
} catch (Throwable $e) {
  jerror(500, 'Error inesperado', ['dev' => $e->getMessage()]);
}
