...
<style>
  ...
  /* ===== Paneles inferiores a ancho completo (2, 3 y 4) ===== */
  .pc-bottom-grid{
    display:flex;
...
#pl-filtros .form-select{
  ...
  .pv-text-layer{position:absolute;inset:0;pointer-events:none}
  .pv-text{
    position:absolute;
    transform:translate(-50%,-50%);
    white-space:normal;
    text-align:center;
    font-size:10pt; /* tamaño base; luego se sobrescribe en JS */
    font-weight:500;
    padding:2px 4px;
  }
  .pv-text-active{
  ...
  #pe-form .pe-col-ej,
  #pe-form td:nth-child(4) {
    width: 30%;
  }

  #pe-form .pe-col-layout,
  #pe-form td:nth-child(5) {
    width: 20%;
    text-align: right;
    white-space: nowrap;
  }
...
<form id="pl-filtros" class="pl-filters-grid">
  ...
  <div class="d-flex flex-column">
    ...
    <!-- NUEVO: COPIAR / ELIMINAR PLANTILLAS -->
    <div class="pc-card" id="pcx-card">
      <div class="pc-title">3. Copiar / eliminar plantillas</div>

      <div id="pcx-empty" class="text-muted">
        Selecciona una plantilla en la lista y usa los botones “Copiar” o “Eliminar” (botones naranja/rojo) para trabajar de forma rápida y segura.
      </div>

      <!-- Panel copiar -->
      <div id="pcx-copy-wrap" class="d-none">
        <div class="mb-2 small text-muted">
          Origen: <strong id="pcx-src-nombre">—</strong> · Empresa: <span id="pcx-src-empresa">—</span> · ID: <span id="pcx-src-id">—</span>
        </div>

        <div id="pcx-copy-alert" class="alert alert-danger d-none"></div>
        <div id="pcx-copy-ok" class="alert alert-success d-none"><span class="msg"></span></div>

        <div class="pc-form-grid">
          <div class="pc-field">
            <label class="form-label mb-1">Empresa destino: <span class="text-danger">*</span></label>
            <select id="pcx-empresa-destino" class="form-select">
              <option value="">Cargando…</option>
            </select>
            <div class="help mt-1">Se copiará la plantilla a la empresa seleccionada.</div>
          </div>

          <div class="pc-field">
            <label class="form-label mb-1">Nombre de la nueva plantilla: <span class="text-danger">*</span></label>
            <input id="pcx-nombre-destino" class="form-control" maxlength="150" placeholder="Ej: CERTIFICADO DE CAPACITACIÓN (Copia)">
            <div class="help mt-1">Debe ser único dentro de la empresa destino.</div>
          </div>

          <div class="pc-field">
            <label class="form-label mb-1">Opciones</label>
            <div class="form-check">
              <input class="form-check-input" type="checkbox" id="pcx-copy-assets" checked>
              <label class="form-check-label" for="pcx-copy-assets">
                Copiar físicamente fondo / logo / firma (recomendado)
              </label>
            </div>
            <div class="help mt-1">Así cada plantilla será independiente y podrás borrarlas sin afectar a otras.</div>
          </div>
        </div>

        <div class="d-flex gap-2 mt-2">
          <button type="button" class="btn btn-warning" id="pcx-copy-go">Crear copia</button>
          <button type="button" class="btn btn-secondary" id="pcx-copy-cancel">Cerrar panel</button>
        </div>
      </div>

      <!-- Panel eliminar -->
      <div id="pcx-del-wrap" class="d-none">
        <div class="mb-2 small text-muted">
          Seleccionada: <strong id="pcx-del-nombre">—</strong> · Empresa: <span id="pcx-del-empresa">—</span> · ID: <span id="pcx-del-id">—</span>
        </div>

        <div id="pcx-del-alert" class="alert alert-danger d-none"></div>
        <div id="pcx-del-ok" class="alert alert-success d-none"><span class="msg"></span></div>

        <div class="alert alert-warning">
          <b>Eliminar definitivamente</b> borrará la plantilla, sus posiciones, elementos y también sus archivos (fondo/logo/firma) si están dentro de <code>almacen/</code>.
        </div>
        ...
        <div class="pc-form-grid">
          <div class="pc-field">
            ...
            <div class="form-check">
              <input class="form-check-input" type="checkbox" id="pcx-del-check">
              <label class="form-check-label" for="pcx-del-check">
                Entiendo que esta acción es irreversible
              </label>
            </div>
            <div class="help mt-1">Esto evita eliminaciones accidentales.</div>
          </div>

          <div class="pc-field">
            <label class="form-label mb-1">Escribe el ID para habilitar “Eliminar”</label>
            <input id="pcx-del-confirm" class="form-control" placeholder="Ej: 12">
            <div class="help mt-1">Debes escribir exactamente el ID mostrado arriba.</div>
          </div>

          <div class="pc-field">
            <label class="form-label mb-1">Acciones</label>
            <div class="d-flex flex-column gap-2">
              <button type="button" class="btn btn-secondary" id="pcx-del-soft">Desactivar (recomendado)</button>
              <button type="button" class="btn btn-danger" id="pcx-del-hard" disabled>Eliminar definitivamente</button>
              <button type="button" class="btn btn-light" id="pcx-del-cancel">Cerrar panel</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="pc-card">
      <div class="pc-title">4. Elementos de certificado</div>
            ...
            <table class="table table-sm align-middle mb-2">
              <thead>
                <tr>
                  ...
                  <th class="pe-col-ej">Ejemplo</th>
                  <th class="pe-col-layout">Posición</th>
                </tr>
              </thead>
              <tbody id="pe-tbody">
                <!-- Se rellena por JS -->
  ...
  <div class="pc-card">
    <div class="pc-title">5. Vista previa</div>
    ...
    <div id="pv-wrap" class="d-none">
        ...
        <div class="pv-group">
          <span class="pv-label">QR:</span>
          <button type="button" class="pv-btn pv-move" title="Arriba (QR)" data-target="qr" data-dy="-1">↑</button>
          <button type="button" class="pv-btn pv-move" title="Abajo (QR)" data-target="qr" data-dy="1">↓</button>
          <button type="button" class="pv-btn pv-move" title="Izquierda (QR)" data-target="qr" data-dx="-1">←</button>
          <button type="button" class="pv-btn pv-move" title="Derecha (QR)" data-target="qr" data-dx="1">→</button>
          <button type="button" class="pv-btn pv-move" title="Más grande (QR)" data-target="qr" data-dw="2">＋</button>
          <button type="button" class="pv-btn pv-move" title="Más pequeño (QR)" data-target="qr" data-dw="-2">－</button>
        </div>

        <div class="pv-group">
          <span class="pv-label">Elemento actual:</span>
          <span id="pv-active-name" class="small text-muted">Ninguno</span>
          <button type="button" class="pv-btn pv-move" title="Arriba (elemento)" data-target="elem" data-dy="-1">↑</button>
          <button type="button" class="pv-btn pv-move" title="Abajo (elemento)" data-target="elem" data-dy="1">↓</button>
          <button type="button" class="pv-btn pv-move" title="Izquierda (elemento)" data-target="elem" data-dx="-1">←</button>
          <button type="button" class="pv-btn pv-move" title="Derecha (elemento)" data-target="elem" data-dx="1">→</button>
          <button type="button" class="pv-btn pv-move" title="Más ancho (elemento)" data-target="elem" data-dw="2">＋</button>
          <button type="button" class="pv-btn pv-move" title="Más angosto (elemento)" data-target="elem" data-dw="-2">－</button>
        </div>
      ...
      <div id="pv-canvas" class="pv-canvas">
        ...
        <img id="pv-qr"    alt="qr"    class="pv-img" style="left:85%;top:20%;width:18%;transform:translate(-50%,-50%);display:none;">
        <div id="pv-text-layer" class="pv-text-layer"></div>
      </div>
      <div class="pv-meta">
