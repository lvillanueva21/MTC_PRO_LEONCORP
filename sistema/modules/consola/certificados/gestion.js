...
export function init(slot, apiUrl) {
  ...
  const DEFAULTS = {
    ...
    qr:    'modules/consola/certificados/img/placeholder-qr.png'
  };

  // Elementos configurables que puede mostrar un certificado (por plantilla)
  const ELEMENTS = [
    {
      code: 'plantilla_nombre',
      label: 'Título de certificado',
      desc: 'Nombre principal de la plantilla de certificado.',
      example: 'CERTIFICADO DE CAPACITACIÓN'
    },
    {
      code: 'plantilla_resolucion',
      label: 'Resolución / norma',
      desc: 'Texto de resolución o norma que respalda el curso.',
      example: 'Res. Directoral N.° 123-2025-MTC/15'
    },
    {
      code: 'nombre_completo',
      label: 'Nombre del participante',
      desc: 'Apellidos y nombres del cliente (previa: apellidos primero).',
      example: 'VILLANUEVA PEREZ LUIGI ISRAEL'
    },
    {
      code: 'curso',
      label: 'Nombre del curso',
      desc: 'Nombre del curso asociado al certificado.',
      example: 'MANEJO DEFENSIVO'
    },
    {
      code: 'detalle_curso',
      label: 'Detalle de participación',
      desc: 'Párrafo con documento, categoría, fechas y horas del curso.',
      example: 'Identificado(a) con DNI N.º 41893357 y categoría AIIIC, ha participado en el curso realizado el 05 de noviembre de 2025, cumpliendo satisfactoriamente con una duración total de 08 horas teóricas y 08 horas prácticas.'
    },
    {
      code: 'codigo_certificado',
      label: 'Código de certificado',
      desc: 'Código único del certificado.',
      example: 'CERT-2025-000123'
    },
    {
      code: 'representante',
      label: 'Representante',
      desc: 'Nombre del representante que suscribe el certificado.',
      example: 'JUAN PEREZ GARCÍA'
    },
    {
      code: 'fecha_caducidad',
      label: 'Fecha de caducidad',
      desc: 'Texto con la fecha de vencimiento del certificado.',
      example: 'Válido hasta: 15/11/2026'
    },
    {
      code: 'ciudad_fecha_emision',
      label: 'Ciudad y fecha de emisión',
      desc: 'Ciudad y fecha de emisión del certificado.',
      example: 'Lima, 15 de noviembre de 2025'
    },
    {
      code: 'qr',
      label: 'Código QR',
      desc: 'QR de verificación del certificado (solo imagen).',
      example: ''
    }
  ];
  ...
  function pcx_hideAll(){
    $('#pcx-empty')?.classList.remove('d-none');
    $('#pcx-copy-wrap')?.classList.add('d-none');
    $('#pcx-del-wrap')?.classList.add('d-none');

    hide($('#pcx-copy-alert')); hide($('#pcx-copy-ok'));
    hide($('#pcx-del-alert'));  hide($('#pcx-del-ok'));
  }

  function pcx_openCopyFromRow(tr){
    if(!tr) return;

    X.mode = 'copy';
    X.sourceId = parseInt(tr.dataset.id,10) || 0;
    X.sourceEmpresaId = parseInt(tr.dataset.id_empresa,10) || 0;
    X.sourceNombre = tr.dataset.nombre || '';
    X.sourceEmpresa = tr.querySelector('td:nth-child(3)')?.textContent?.trim() || '';

    pcx_hideAll();
    $('#pcx-empty')?.classList.add('d-none');
    $('#pcx-copy-wrap')?.classList.remove('d-none');

    const lblId = $('#pcx-src-id'); if(lblId) lblId.textContent = String(X.sourceId || '—');
    const lblN  = $('#pcx-src-nombre'); if(lblN) lblN.textContent = X.sourceNombre || '—';
    const lblE  = $('#pcx-src-empresa'); if(lblE) lblE.textContent = X.sourceEmpresa || '—';

    const selDest = $('#pcx-empresa-destino');
    if (selDest) selDest.value = String(X.sourceEmpresaId || '');

    const nameDest = $('#pcx-nombre-destino');
    if (nameDest) nameDest.value = (X.sourceNombre ? (X.sourceNombre + ' (Copia)') : '');

    const chk = $('#pcx-copy-assets');
    if (chk) chk.checked = true;

    $('#pcx-card')?.scrollIntoView({behavior:'smooth', block:'start'});
  }

  function pcx_openDeleteFromRow(tr){
    if(!tr) return;
    ...
    pcx_hideAll();
    $('#pcx-empty')?.classList.add('d-none');
    $('#pcx-del-wrap')?.classList.remove('d-none');

    const lblId = $('#pcx-del-id'); if(lblId) lblId.textContent = String(X.sourceId || '—');
    const lblN  = $('#pcx-del-nombre'); if(lblN) lblN.textContent = X.sourceNombre || '—';
    const lblE  = $('#pcx-del-empresa'); if(lblE) lblE.textContent = X.sourceEmpresa || '—';

    const chk = $('#pcx-del-check'); if(chk) chk.checked = false;
    const inp = $('#pcx-del-confirm'); if(inp) inp.value = '';

    const hardBtn = $('#pcx-del-hard');
    if (hardBtn) hardBtn.disabled = true;

    $('#pcx-card')?.scrollIntoView({behavior:'smooth', block:'start'});
  }

  function pcx_updateHardDeleteButton(){
    const chk = $('#pcx-del-check');
    const inp = $('#pcx-del-confirm');
    const btn = $('#pcx-del-hard');
    if (!btn) return;

    const ok1 = chk && chk.checked;
    const ok2 = inp && String(inp.value || '').trim() === String(X.sourceId || '');
  ...
  // ---------- Combos (empresas) ----------
    ...
    try{
      ...
      const selForm = $('#pc-empresa');
      if(selForm){ selForm.innerHTML = `<option value="">Seleccione…</option>${opts}`; }

      const selList = $('#pl-empresa');
      if(selList){ selList.innerHTML = `<option value="">Todas</option>${opts}`; }

      const selCopy = $('#pcx-empresa-destino');
      if(selCopy){ selCopy.innerHTML = `<option value="">Seleccione…</option>${opts}`; }
    }catch(e){
  ...
  function pintarTabla(rows){
    ...
    $('#pl-empty')?.classList.add('d-none');
      ...
      const isActive = String(r.activo ?? '1') === '1';
      const nameHtml = isActive
        ? esc(r.nombre)
        : `${esc(r.nombre)} <span class="badge badge-secondary ms-1">Inactivo</span>`;
      ...
      <tr data-id="${r.id}"
          ...
          data-activo="${esc(String(r.activo ?? '1'))}">
        <td>${r.id}</td>
        <td>${nameHtml}</td>
        <td>${esc(r.empresa)}</td>
        <td>${r.paginas}</td>
        ...
        <td class="text-nowrap">
          ...
          <button class="btn btn-sm btn-warning pl-copy" title="Copiar plantilla" aria-label="Copiar plantilla">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                 fill="currentColor" viewBox="0 0 16 16" aria-hidden="true">
              <path d="M10 1.5H4A1.5 1.5 0 0 0 2.5 3v8A1.5 1.5 0 0 0 4 12.5h6A1.5 1.5 0 0 0 11.5 11V3A1.5 1.5 0 0 0 10 1.5z"/>
              <path d="M12 3.5h.5A1.5 1.5 0 0 1 14 5v8A1.5 1.5 0 0 1 12.5 14.5H6A1.5 1.5 0 0 1 4.5 13v-.5H10A2.5 2.5 0 0 0 12.5 10V3.5z"/>
            </svg>
          </button>

          <button class="btn btn-sm btn-danger pl-del" title="Eliminar / desactivar" aria-label="Eliminar / desactivar">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                 fill="currentColor" viewBox="0 0 16 16" aria-hidden="true">
              <path d="M5.5 5.5A.5.5 0 0 1 6 5h4a.5.5 0 0 1 .5.5v8a.5.5 0 0 1-1 0V6H6v7.5a.5.5 0 0 1-1 0v-8z"/>
              <path d="M14.5 3a1 1 0 0 1-1 1H13v10a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1 0-2H5.5l1-1h3l1 1h3a1 1 0 0 1 1 1zM4 4v10a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4H4z"/>
            </svg>
          </button>
        </td>
      </tr>
  ...
  // ---------- Panel 3: Elementos de certificado ----------
    ...
    tbody.innerHTML = ELEMENTS.map(el => {
      // QR: solo checkbox y descripción, sin controles de texto
      if (el.code === 'qr') {
        return `
          <tr data-code="${esc(el.code)}">
            ...
            <td class="text-muted small">
              La posición y tamaño del QR se ajustan en la barra de herramientas de la vista previa.
            </td>
            <td class="text-end">
        ...
        <tr data-code="${esc(el.code)}">
          ...
          <td>
            ...
            <div class="mt-1 d-flex flex-wrap gap-1 align-items-center">
              ...
              <input type="color" class="form-control form-control-color pe-color ms-1"
                     value="#000000" title="Color del texto">
            </div>
          </td>
          <td class="text-end">
    ...
    tbody.querySelectorAll('tr').forEach(tr => {
      ...
      const inpColor = tr.querySelector('.pe-color');
      ...
      if (inpColor) {
        if (fromLayout && fromLayout.color) {
          inpColor.value = fromLayout.color;
        } else {
    ...
    try {
      ...
      // Guardamos también en PV para que la vista previa lo use aunque el panel 3 no esté abierto
      PV.visibleCodes = active;
      ...
      // Actualizar preview según selección actual
      pv_renderTexts();
      pv_renderQr();
    } catch (err) {
  ...
  const PV = {
    ...
    qr:    { x:85, y:20, w:18 },  // QR como imagen independiente
    // elements[code] = { x,y,w,texto,size,bold,align,font,color }
    elements: {},
    activeElement: '',
    visibleCodes: new Set()       // elementos marcados en "Ver" para la plantilla actual
  };
  ...
  function pv_applyStyles(){
    ...
    if (elQr){
      elQr.style.left  = `${pv_clamp(Lqr.x,0,100)}%`;
      elQr.style.top   = `${pv_clamp(Lqr.y,0,100)}%`;
      elQr.style.width = `${pv_clamp(Lqr.w,5,90)}%`;
    }
  }
  ...
  function pv_getActiveCodes(){
    // Base: lo que vino del servidor (elements_get)
    const base = (PV.visibleCodes instanceof Set) ? PV.visibleCodes : new Set();

    const tbody = $('#pe-tbody');
    // Si el panel 3 no está cargado (sin filas), usamos sólo lo que vino de BD
    if (!tbody || !tbody.querySelector('tr')) {
      return new Set(base);
    }

    // Si el panel 3 está cargado, lo que manda es el estado actual de los checkboxes
    const set = new Set();
    tbody.querySelectorAll('.pe-check:checked').forEach(ch => {
      if (ch.value) set.add(ch.value);
    });
    return set;
  }

  function pv_isCodeActive(code){
    if (!code) return false;
    const set = pv_getActiveCodes();
    return set.has(code);
  }

  function pv_renderQr(){
    const img = $('#pv-qr');
    if (!img) return;

    // Mostrar u ocultar según checkbox "QR"
    const active = pv_isCodeActive('qr');
    if (!active){
    ...
    const cfg = PV.qr || { x:85, y:20, w:18 };
    const x = pv_clamp(cfg.x ?? 85, 0, 100);
    const y = pv_clamp(cfg.y ?? 20, 0, 100);
    const w = pv_clamp(cfg.w ?? 18, 5, 90);
    ...
    const active = pv_getActiveCodes();

    Object.keys(elems).forEach(code => {
      // El QR se maneja como imagen, no texto
      if (code === 'qr') return;

      // Si hay selección de elementos, solo mostramos los que están marcados
      if (active.size && !active.has(code)) return;
    ...
    // Fuente
      ...
      const color = (cfg.color && String(cfg.color).trim()) || '#000000';
    ...
    PV.qr    = { x:85, y:20, w:18 };
    PV.elements = {};
    PV.activeElement = '';
    PV.visibleCodes = new Set();
  ...
  function loadImage(src){
    ...
    // Cargar desde el servidor qué elementos están marcados como "visibles"
    try {
      const rElems = await j(`${apiUrl}?action=elements_get&id=${encodeURIComponent(PV.row.id)}`);
      const arr = Array.isArray(rElems.data) ? rElems.data : [];
      PV.visibleCodes = new Set(arr);
    } catch (e) {
      PV.visibleCodes = new Set();
    }

    const wrap = $('#pv-wrap'), empty = $('#pv-empty'), canvas = $('#pv-canvas');
    const elLogo  = $('#pv-logo'), elFirma = $('#pv-firma'), elQr = $('#pv-qr');
    if(!wrap || !canvas || !elLogo || !elFirma || !elQr){ return; }
    ...
    const qrUrl    = assetUrl(DEFAULTS.qr);
    ...
    await Promise.all([ loadImage(logoUrl), loadImage(firmaUrl), loadImage(qrUrl) ]);
    pv_showLoader(false);
    ...
    // Botón imprimir PDF (pasa posiciones de logo/firma por querystring; QR se lee desde layout)
  ...
  slot.addEventListener('click', e=>{
    ...
    if (tgt === 'logo' || tgt === 'firma' || tgt === 'qr') {
      // Para logo/firma/qr dw sigue siendo ancho (%)
    ...
    const texto = input && input.value.trim() !== ''
      ? input.value
      : pv_defaultExampleFor(code);

    const colorVal = inpColor && inpColor.value ? inpColor.value : '#000000';
    ...
    const lbl = $('#pv-active-name');
    const def = ELEMENTS.find(el => el.code === code);
    if (lbl) lbl.textContent = (def && def.label) ? def.label : code;

    pv_renderTexts();
    pe_syncRow(tr); // ya hay layout → quitamos el "pendiente"
  ...
  // Guardar posiciones de logo, firma, QR y elementos en BD
  slot.addEventListener('click', async e=>{
    ...
    tbody.querySelectorAll('tr').forEach(tr => {
      ...
      if (!check) return;
      const code = check.value;
      if (!code || code === 'qr') return; // QR va por PV.qr, no como texto

      const base = PV.elements[code] || {
        x: 50,
        y: 50,
        w: 40,
        texto: pv_defaultExampleFor(code),
        size: 100,
        bold: false,
        align: 'C',
        font: '',
      ...
      const texto = input && input.value.trim() !== ''
        ? input.value
        : (base.texto || pv_defaultExampleFor(code));
      ...
      const color = inpColor && inpColor.value ? inpColor.value : (base.color || '#000000');
  ...
  slot.addEventListener('click', e=>{
    ...
    // Abrir panel Copiar desde la tabla
  slot.addEventListener('click', e=>{
    const btn = e.target?.closest('.pl-copy'); if(!btn) return;
    const tr = btn.closest('tr'); if(!tr) return;
    pcx_openCopyFromRow(tr);
  });

  // Abrir panel Eliminar/Desactivar desde la tabla
  slot.addEventListener('click', e=>{
    const btn = e.target?.closest('.pl-del'); if(!btn) return;
    const tr = btn.closest('tr'); if(!tr) return;
    pcx_openDeleteFromRow(tr);
  });
  ...
  slot.addEventListener('input', debounce(e=>{
    ...
    if (t.classList && t.classList.contains('pe-check')) {
      ...
      const code = t.value;
      if (code) {
        if (!(PV.visibleCodes instanceof Set)) {
          PV.visibleCodes = new Set();
        }
        if (t.checked) {
          PV.visibleCodes.add(code);
        } else {
          PV.visibleCodes.delete(code);
        }
      }

      // Actualizar vista previa cuando se marca / desmarca un elemento
      pv_renderTexts();
      pv_renderQr();
    }
  });

  // ---------- Acciones del panel Copiar / Eliminar ----------
  slot.addEventListener('click', async e=>{
    if (!e.target?.closest('#pcx-copy-go')) return;
    const btn = e.target.closest('#pcx-copy-go');
    btn.disabled = true;
    ...
    try {
      if (!X.sourceId) throw new Error('No hay plantilla origen seleccionada.');

      const empresaDestino = $('#pcx-empresa-destino')?.value || '';
      const nombreDestino  = ($('#pcx-nombre-destino')?.value || '').trim();
      const copyAssets     = $('#pcx-copy-assets')?.checked ? 1 : 0;

      if (!empresaDestino) throw new Error('Selecciona la empresa destino.');
      if (!nombreDestino)  throw new Error('Escribe el nombre de la nueva plantilla.');

      const fd = new FormData();
      fd.append('action', 'copy');
      fd.append('source_id', String(X.sourceId));
      fd.append('target_empresa_id', String(empresaDestino));
      fd.append('nombre', nombreDestino);
      fd.append('copy_assets', String(copyAssets));

      const r = await j(apiUrl, { method:'POST', body: fd });

      show($('#pcx-copy-ok'), `Plantilla copiada. Nueva ID: ${r.id || '—'}`);
      // refrescar listado
      await pl_cargarLista();
    } catch (err) {
      show($('#pcx-copy-alert'), err.message || 'Error al copiar.');
    } finally {
      btn.disabled = false;
    }
  });
  ...
  slot.addEventListener('click', e=>{
    if (!e.target?.closest('#pcx-copy-cancel')) return;
    pcx_hideAll();
  });

  slot.addEventListener('click', async e=>{
    if (!e.target?.closest('#pcx-del-soft')) return;

    const btn = e.target.closest('#pcx-del-soft');
    btn.disabled = true;
    hide($('#pcx-del-alert'));
    hide($('#pcx-del-ok'));

    try {
      if (!X.sourceId) throw new Error('No hay plantilla seleccionada.');

      const fd = new FormData();
      fd.append('action', 'deactivate');
      fd.append('id', String(X.sourceId));

      await j(apiUrl, { method:'POST', body: fd });
      show($('#pcx-del-ok'), 'Plantilla desactivada.');
      await pl_cargarLista();
    } catch (err) {
      show($('#pcx-del-alert'), err.message || 'Error al desactivar.');
    } finally {
      btn.disabled = false;
    }
  });
  ...
  slot.addEventListener('input', e=>{
    if (e.target?.id === 'pcx-del-confirm') {
      pcx_updateHardDeleteButton();
    }
  });

  slot.addEventListener('change', e=>{
    if (e.target?.id === 'pcx-del-check') {
      pcx_updateHardDeleteButton();
    }
  });

  slot.addEventListener('click', async e=>{
    if (!e.target?.closest('#pcx-del-hard')) return;

    const btn = e.target.closest('#pcx-del-hard');
    btn.disabled = true;
    hide($('#pcx-del-alert'));
    hide($('#pcx-del-ok'));
    ...
    try {
      ...
      // seguridad extra
      const chk = $('#pcx-del-check')?.checked;
      const inp = String($('#pcx-del-confirm')?.value || '').trim();
      if (!chk) throw new Error('Debes marcar la confirmación.');
      if (inp !== String(X.sourceId)) throw new Error('El ID escrito no coincide.');
      ...
      const r = await j(apiUrl, { method:'POST', body: fd });
      show($('#pcx-del-ok'), r.msg || 'Plantilla eliminada definitivamente.');

      // cerrar panel y refrescar
      pcx_hideAll();
      await pl_cargarLista();
      pv_reset();
    } catch (err) {
      show($('#pcx-del-alert'), err.message || 'Error al eliminar.');
    } finally {
      btn.disabled = false;
      pcx_updateHardDeleteButton();
    }
  ...
  slot.addEventListener('click', e=>{
    if (!e.target?.closest('#pcx-del-cancel')) return;
    pcx_hideAll();
  });
  ...
  slot.__pcRefresh = async function(){
    ...
    // nuevo: reset del panel copiar/eliminar
    pcx_hideAll();
