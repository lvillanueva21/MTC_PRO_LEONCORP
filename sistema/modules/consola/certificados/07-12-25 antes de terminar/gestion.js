...
export function init(slot, apiUrl) {
  ...
  async function pe_loadForTemplate(tplId, tplNombre, tplEmpresa){
    ...
    // Pintar filas base (todos desmarcados)
    ...
    // Cargar selección actual desde API
    try {
      ...
      tbody.querySelectorAll('.pe-check').forEach(ch => {
        if (active.has(ch.value)) ch.checked = true;
      });
    } catch (err) {
  ...
  async function pv_loadFromRow(tr){
    ...
    // Intentar cargar posiciones guardadas desde el API
    try {
      ...
      // d.elements queda listo para usar más adelante si quieres dibujar textos
    } catch (e) {
  ...
  slot.addEventListener('click', async e=>{
    ...
    try {
      const payload = {
        logo:  PV.logo,
        firma: PV.firma
        // Más adelante puedes añadir aquí PV.elements si decides editarlos desde la vista previa
      };
  ...
  // Abrir configuración de elementos (panel 3) desde la tabla
  slot.addEventListener('click', e => {
