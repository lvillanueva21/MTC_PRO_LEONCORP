...
try {
  ...
  jok(['data'=>$rows,'total'=>$total,'page'=>$page,'per_page'=>$perPage]);
    ...
    case 'layout_get': {
      ...
      $st2 = $db->prepare("
        SELECT codigo_elemento, pos_x, pos_y, ancho
    ...
    case 'layout_save': {
      ...
      // Otros elementos (posición; ancho = 0)
        ...
        if ($rows) {
          $stIns = $db->prepare("
            INSERT INTO cq_plantillas_posiciones
              (id_plantilla_certificado, codigo_elemento, pagina, pos_x, pos_y, ancho)
            VALUES (?,?,?,?,?,?)
          ");
          foreach ($rows as $r) {
            $code   = $r['code'];
            ...
            $y      = $r['y'];
            $w      = $r['w'];
            $stIns->bind_param('isiddd', $id, $code, $pagina, $x, $y, $w);
            $stIns->execute();
          }
          $stIns->close();
        }
