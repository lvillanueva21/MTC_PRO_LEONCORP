...
<?php require __DIR__ . '/modales.php'; ?>
...
(function() {
  ...
  function vincularEventosPaginacion() {
    ...
    botonesEdit.forEach(function(btn) {
      btn.addEventListener('click', function(e) {
        ...
        // Botón cambiar estado (activar / desactivar) con modal de confirmación
    var botonesDesactivar = tbody.querySelectorAll('.btn-icon-deactivate[data-cert-id]');
    botonesDesactivar.forEach(function(btn) {
      btn.addEventListener('click', function(e) {
        e.preventDefault();
        var id = parseInt(this.getAttribute('data-cert-id'), 10) || 0;
        if (!id) {
        ...
        abrirModalCambioEstadoCertificado(id);
      });
    });
  ...
  function limpiarModalCambioEstadoCertificado() {
    var err = document.getElementById('modalConfirmEstadoCertError');
    if (err) {
      err.style.display = 'none';
      err.textContent = '';
    }

    var msg = document.getElementById('modalConfirmEstadoCertMensaje');
    if (msg) {
      msg.textContent = '';
    }

    var camposTexto = [
      'modalConfirmEstadoCertCodigo',
      'modalConfirmEstadoCertAlumno',
      'modalConfirmEstadoCertCurso',
      'modalConfirmEstadoCertEstado'
    ];
    ...
    var idInput = document.getElementById('modalConfirmEstadoCertId');
    if (idInput) {
      idInput.value = '';
    }

    var accionInput = document.getElementById('modalConfirmEstadoCertAccion');
    if (accionInput) {
      accionInput.value = '';
    }
  }

  function mostrarErrorModalCambioEstado(msg) {
    var err = document.getElementById('modalConfirmEstadoCertError');
    if (err) {
  ...
  function abrirModalCambioEstadoCertificado(idCert) {
    limpiarModalCambioEstadoCertificado();

    fetch('detalle_certificado.php?id=' + encodeURIComponent(idCert), {
      method: 'GET',
      credentials: 'same-origin'
    })
    .then(function(r) { return r.json(); })
    .then(function(data) {
      if (!data || !data.ok || !data.resumen) {
        mostrarErrorModalCambioEstado('No se pudieron obtener los datos del certificado.');
        return;
      }

      var resumen = data.resumen || {};

      var estadoBd = (resumen.estado_bd || '').toString();
      var estadoActualUpper = estadoBd.toUpperCase();

      var accion = 'desactivar';
      var header = document.getElementById('modalConfirmEstadoCertHeader');
      var btnConfirm = document.getElementById('btnConfirmEstadoCert');

      if (estadoActualUpper === 'INACTIVO') {
        accion = 'activar';
      }

      var mensaje = (accion === 'activar')
        ? '¿Seguro que deseas activar este certificado?'
        : '¿Seguro que deseas desactivar este certificado?';

      var msgEl = document.getElementById('modalConfirmEstadoCertMensaje');
      if (msgEl) {
        msgEl.textContent = mensaje;
      }

      var codigoEl = document.getElementById('modalConfirmEstadoCertCodigo');
      if (codigoEl) {
        codigoEl.textContent = resumen.codigo_certificado || '';
      }

      var alumnoEl = document.getElementById('modalConfirmEstadoCertAlumno');
      if (alumnoEl) {
        alumnoEl.textContent = resumen.nombre_cliente || '';
      }

      var cursoEl = document.getElementById('modalConfirmEstadoCertCurso');
      if (cursoEl) {
        cursoEl.textContent = resumen.curso || '';
      }

      var estadoEl = document.getElementById('modalConfirmEstadoCertEstado');
      if (estadoEl) {
        estadoEl.textContent = estadoBd || '';
      }

      var idInput = document.getElementById('modalConfirmEstadoCertId');
      if (idInput) {
        idInput.value = idCert;
      }
      ...
      if (btnConfirm) {
        btnConfirm.classList.remove('btn-danger', 'btn-success');
        if (accion === 'activar') {
          btnConfirm.classList.add('btn-success');
          btnConfirm.textContent = 'Activar certificado';
        } else {
          btnConfirm.classList.add('btn-danger');
          btnConfirm.textContent = 'Desactivar certificado';
        }
      }

      if (window.jQuery && typeof jQuery.fn.modal === 'function') {
        jQuery('#modalConfirmEstadoCert').modal('show');
      } else {
        var m = document.getElementById('modalConfirmEstadoCert');
        if (m) {
    ...
    .catch(function(err) {
      console.error(err);
      mostrarErrorModalCambioEstado('Error de comunicación al obtener los datos del certificado.');
    });
  }
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    var btnConfirmEstado = document.getElementById('btnConfirmEstadoCert');
    if (btnConfirmEstado) {
      btnConfirmEstado.addEventListener('click', function(e) {
        e.preventDefault();

        var idInput = document.getElementById('modalConfirmEstadoCertId');
        if (!idInput || !idInput.value) {
          mostrarErrorModalCambioEstado('No se pudo determinar el certificado seleccionado.');
          return;
        }

        var idCert = parseInt(idInput.value, 10) || 0;
        if (!idCert) {
          mostrarErrorModalCambioEstado('No se pudo determinar el certificado seleccionado.');
          return;
        }
        ...
        btnConfirmEstado.disabled = true;

        fetch('desactivar_certificado.php', {
          method: 'POST',
          body: fd,
          credentials: 'same-origin'
        })
        ...
        .then(function(data) {
          if (!data || typeof data !== 'object') {
            mostrarErrorModalCambioEstado('Respuesta inesperada del servidor al cambiar el estado.');
            return;
          }

          if (!data.ok) {
            var msg = data.error || 'No se pudo cambiar el estado del certificado.';
            mostrarErrorModalCambioEstado(msg);
            return;
          }

          if (window.jQuery && typeof jQuery.fn.modal === 'function') {
            jQuery('#modalConfirmEstadoCert').modal('hide');
          } else {
            var m = document.getElementById('modalConfirmEstadoCert');
            if (m) {
        ...
        .catch(function(err) {
          console.error(err);
          mostrarErrorModalCambioEstado('Error de comunicación con el servidor.');
        })
        .then(function() {
          btnConfirmEstado.disabled = false;
        });
      });
    }
  });
