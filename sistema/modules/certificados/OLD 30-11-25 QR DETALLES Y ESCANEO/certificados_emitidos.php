...
(function() {
  function obtenerFiltrosListado() {
    ...
    .then(function(data) {
      ...
      // Botones QR: abrir detalle usando el mismo modal del formulario
      if (tbody) {
        var qrButtons = tbody.querySelectorAll('.btn-icon-qr[data-id]');
        qrButtons.forEach(function(btn) {
          btn.addEventListener('click', function(e) {
            ...
            if (!window.abrirModalDetalleCertificado) {
              alert('No se encuentra la función para mostrar el detalle del certificado.');
              return;
            }
            ...
            .then(function(r) { return r.json(); })
            .then(function(resp) {
              if (!resp || typeof resp !== 'object' || !resp.ok || !resp.resumen) {
                alert('No se pudo obtener el detalle del certificado.');
                return;
              }
              window.abrirModalDetalleCertificado(resp.resumen);
            })
            .catch(function(err) {
              console.error(err);
              alert('Error al obtener el detalle del certificado.');
            });
          });
        });
      }
