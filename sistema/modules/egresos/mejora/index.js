...
function voucherFuentesHtml(v){
  ...
  if(!groups.length){
    return `
      <div class="egpv3-source-group">
        <div class="egpv3-source-group-head">Sin fuentes</div>
        <div class="egpv3-source-row">
          <div class="egpv3-source-name">Sin fuentes registradas</div>
  ...
  return groups.map(g=>{
    const cajaCodigo=(g&&g.caja_diaria_codigo)||('Caja '+String(g.id_caja_diaria||''));
    const cajaFecha=(g&&g.caja_diaria_fecha)?fmtDate(g.caja_diaria_fecha):'';
    const titulo=cajaFecha ? `${cajaCodigo} · ${cajaFecha}` : cajaCodigo;

    const rows=(Array.isArray(g.rows)?g.rows:[]).map(item=>{
      const lbl=(item&&item.label)||FUENTE_LABEL[canonKey((item&&item.key)||'')]||canonKey((item&&item.key)||'')||'Fuente';
      return `
        <div class="egpv3-source-row">
          <div class="egpv3-source-name">${esc(lbl)}</div>
          <div class="egpv3-source-amount">${esc(money((item&&item.monto)||0))}</div>
        </div>
      `;
    }).join('');
    ...
    return `
      <div class="egpv3-source-group">
        <div class="egpv3-source-group-head">${esc(titulo)}</div>
        ${rows}
        <div class="egpv3-source-row egpv3-source-total">
          <div class="egpv3-source-name">Total caja</div>
          <div class="egpv3-source-amount">${esc(money((g&&g.total)||0))}</div>
        </div>
      </div>
    `;
  }).join('');
...
function voucherHTML(v,logo){
  ...
  const showBeneficiarioFirma=String(v.tipo_comprobante||'RECIBO').toUpperCase()==='RECIBO';

  const logoHtml=logo
    ? `<div class="egpv3-logo"><img src="${esc(logo)}" alt="Logo"></div>`
    : `<div class="egpv3-logo"><span>LOGO</span></div>`;

  const firmasHtml=showBeneficiarioFirma
    ? `
    ...
    `
    : `
      <div class="egpv3-signatures egpv3-signatures-single">
        <div class="egpv3-sign">
          <div class="egpv3-sign-line"></div>
          <div class="egpv3-sign-name">${esc(responsable)}</div>
          <div class="egpv3-sign-role">Responsable</div>
  ...
  return `
    <style>
      ...
      #egVoucher .egpv3-band{
        ...
        padding:10px 14px;
        display:grid;
        grid-template-columns:1fr 1.35fr .82fr .82fr;
        gap:12px;
        align-items:start;
      }
      ...
      #egVoucher .egpv3-submeta{
        margin-top:8px;
        display:grid;
        grid-template-columns:1fr 1fr;
        gap:10px;
      }
      #egVoucher .egpv3-submeta-box{
        border-top:1px solid #d0d3d6;
        padding-top:6px;
      }
      #egVoucher .egpv3-submeta-label{
        color:#2f4868;
        font-size:12px;
        font-weight:800;
        line-height:1.15;
      }
      #egVoucher .egpv3-submeta-value{
        margin-top:3px;
        font-size:13px;
        line-height:1.25;
        word-break:break-word;
      ...
      #egVoucher .egpv3-source-group{
        margin-bottom:10px;
      }
      #egVoucher .egpv3-source-group:last-child{
        margin-bottom:0;
      }
      #egVoucher .egpv3-source-group-head{
        background:#e6e9ee;
        color:#2f4868;
      ...
      #egVoucher .egpv3-signatures-single{
    ...
    <div class="egpv3-wrap">
      ...
      <div class="egpv3-band">
        ...
        <div class="egpv3-band-item">
          <div class="egpv3-band-label">Tipo</div>
          <div class="egpv3-band-value">${esc(tipoEgreso)}</div>
        </div>
        <div class="egpv3-band-item">
      ...
      <div class="egpv3-submeta">
        <div class="egpv3-submeta-box">
          <div class="egpv3-submeta-label">Caja de registro</div>
          <div class="egpv3-submeta-value">${esc(cajaRegistro)}${cajaRegistroFecha && cajaRegistroFecha !== '-' ? ' · '+esc(cajaRegistroFecha) : ''}</div>
        </div>
        <div class="egpv3-submeta-box">
          <div class="egpv3-submeta-label">Responsable</div>
          <div class="egpv3-submeta-value">${esc(responsable)}</div>
        </div>
      </div>
