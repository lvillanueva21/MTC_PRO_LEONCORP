...
if ($accion === 'listar') {
    ...
    $tipoEgreso = eg_tipo_egreso_label((string)($eg['tipo_egreso'] ?? 'NORMAL'));
    ...
    $cajaRegistro = eg_pdf_clean_text((string)($eg['caja_diaria_codigo'] ?? ''));
    $cajaRegistroFecha = eg_human_ymd((string)($eg['caja_diaria_fecha'] ?? ''));

    if ($beneficiario === '') $beneficiario = '-';
    if ($documento === '') $documento = '-';
    if ($concepto === '') $concepto = '-';
    if ($cajaRegistro === '') $cajaRegistro = '-';
    ...
    $groupedFuentes = eg_group_fuentes_by_caja($egFuentes);
    if (!$groupedFuentes) {
        $groupedFuentes = [[
            'id_caja_diaria' => 0,
            'caja_diaria_codigo' => 'Sin fuentes',
            'caja_diaria_fecha' => '',
            'rows' => [[
                'label' => 'Sin fuentes registradas',
                'monto' => 0,
            ]],
            'total' => 0,
        ]];
    ...
    $bandCompW = 58.0;
    $bandTipoW = 28.0;
    $bandStateW = $bandContentW - $bandDateW - $bandCompW - $bandTipoW - ($bandGap * 3);
    ...
    $tipoLines = eg_pdf_wrap_text_lines($pdf, 'Tipo: ' . $tipoEgreso, $bandTipoW);
    $stateLines = eg_pdf_wrap_text_lines($pdf, 'Estado: ' . $estado, $bandStateW);
    $bandRows = max(count($dateLines), count($compLines), count($tipoLines), count($stateLines));
    ...
    // Submeta (caja + responsable)
    $metaY = $infoY + $infoH + 5.2;
    $metaGap = 8.0;
    $metaLeftW = 108.0;
    $metaRightW = $innerW - $metaLeftW - $metaGap;
    $metaLabelH = 4.2;
    $metaLineH = 4.6;
    $cajaText = trim($cajaRegistro . ($cajaRegistroFecha !== '' ? ' | ' . $cajaRegistroFecha : ''));
    $pdf->SetFont('dejavusans', '', 9.2);
    $cajaLines = eg_pdf_wrap_text_lines($pdf, $cajaText, $metaLeftW);
    $respLines = eg_pdf_wrap_text_lines($pdf, $responsable, $metaRightW);
    $metaH = max(
        $metaLabelH + 1.4 + (count($cajaLines) * $metaLineH),
        $metaLabelH + 1.4 + (count($respLines) * $metaLineH)
    );

    // Concepto
    $conceptY = $metaY + $metaH + 6.0;
    $conceptLabelW = 22.0;
    ...
    $sourceLineGap = 1.5;
    $groupHeadH = 7.0;
    $sourceLabelW = $sourcesW - 26.0;
    $sourceAmountW = 20.0;

    $preparedGroups = [];
    $sourcesRowsH = 0.0;

    $pdf->SetFont('dejavusans', '', 8.9);
    foreach ($groupedFuentes as $group) {
        $groupTitle = trim((string)($group['caja_diaria_codigo'] ?? 'Caja'));
        $groupFecha = trim((string)($group['caja_diaria_fecha'] ?? ''));
        if ($groupFecha !== '') {
            $groupTitle .= ' | ' . eg_human_ymd($groupFecha);
        }

        $groupTitleLines = eg_pdf_wrap_text_lines($pdf, $groupTitle, $sourcesW - 6.0);
        $groupHeadRealH = max($groupHeadH, count($groupTitleLines) * 3.8 + 2.0);

        $rowsPrepared = [];
        $groupRowsH = 0.0;

        foreach ((array)($group['rows'] ?? []) as $src) {
            ...
            $rowH = max(5.8, count($labelLines) * 4.2);
            ...
            $groupRowsH += $rowH + $sourceLineGap;
        }

        $totalText = 'S/. ' . number_format((float)($group['total'] ?? 0), 2, '.', ',');
        $preparedGroups[] = [
            'title_lines' => $groupTitleLines,
            'title_h' => $groupHeadRealH,
            'rows' => $rowsPrepared,
            'total' => $totalText,
        ];

        $sourcesRowsH += $groupHeadRealH + $groupRowsH + 7.0;
    }
    ...
    eg_pdf_draw_text_lines($pdf, $col3X, $bandTextY, $bandTipoW, $tipoLines, $bandLineH, 'L');
    eg_pdf_draw_text_lines($pdf, $col4X, $bandTextY, $bandStateW, $stateLines, $bandLineH, 'L');
    ...
    // Caja registro / responsable
    $pdf->SetTextColor($colBrand[0], $colBrand[1], $colBrand[2]);
    $pdf->SetFont('dejavusans', 'B', 9.0);

    $metaLeftX = $innerX;
    $metaRightX = $innerX + $metaLeftW + $metaGap;

    $pdf->SetXY($metaLeftX, $metaY);
    $pdf->Cell($metaLeftW, 4.2, 'CAJA REGISTRO', 0, 0, 'L');

    $pdf->SetXY($metaRightX, $metaY);
    $pdf->Cell($metaRightW, 4.2, 'RESPONSABLE', 0, 0, 'L');

    $pdf->SetTextColor($colInk[0], $colInk[1], $colInk[2]);
    $pdf->SetFont('dejavusans', '', 9.2);

    eg_pdf_draw_text_lines($pdf, $metaLeftX, $metaY + 5.2, $metaLeftW, $cajaLines, $metaLineH, 'L');
    eg_pdf_draw_text_lines($pdf, $metaRightX, $metaY + 5.2, $metaRightW, $respLines, $metaLineH, 'L');
    ...
    foreach ($preparedGroups as $group) {
        $pdf->SetFillColor($colSoftLight[0], $colSoftLight[1], $colSoftLight[2]);
        $pdf->SetDrawColor(200, 206, 214);
        eg_pdf_round_rect($pdf, $sourcesX, $sourceY, $sourcesW, $group['title_h'], 1.8, 'DF');

        $pdf->SetTextColor($colBrand[0], $colBrand[1], $colBrand[2]);
        $pdf->SetFont('dejavusans', 'B', 7.9);
        eg_pdf_draw_text_lines($pdf, $sourcesX + 3.0, $sourceY + 1.3, $sourcesW - 6.0, $group['title_lines'], 3.8, 'L');

        $sourceY += $group['title_h'] + 1.5;

        $pdf->SetTextColor($colInk[0], $colInk[1], $colInk[2]);
        $pdf->SetFont('dejavusans', '', 8.9);

        foreach ($group['rows'] as $src) {
            $rowH = (float)$src['height'];

            eg_pdf_draw_text_lines($pdf, $sourcesX + 2.0, $sourceY, $sourceLabelW, $src['label_lines'], 4.2, 'L');
        ...
        $pdf->SetFont('dejavusans', 'B', 8.9);
        $pdf->SetXY($sourcesX + 2.0, $sourceY);
        $pdf->Cell($sourceLabelW, 5.2, 'Total caja', 0, 0, 'L');
        $pdf->SetXY($sourcesX + $sourcesW - $sourceAmountW - 1.0, $sourceY);
        $pdf->Cell($sourceAmountW, 5.2, $group['total'], 0, 0, 'R');
